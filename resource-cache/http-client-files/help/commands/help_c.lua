...
function F1RPhelp( key, keyState )
	if not myWindow then
		...
		myWindow = guiCreateWindow ( 399, 240, 583, 361, "Los Santos RolePlay - F1 Help", false )
		exports.global:centerWindow(myWindow)
		guiWindowSetSizable(myWindow, false)
		local tabPanel = guiCreateTabPanel ( 9, 23, 564, 328, false, myWindow )
		...
		local tabRules = guiCreateTab( "[+]Reglement", tabPanel )
		...
		local tabExplained = guiCreateTab( "[+]Dev Logs", tabPanel )
		...
		local tabCommands = guiCreateTab( "[+]Cmds", tabPanel )

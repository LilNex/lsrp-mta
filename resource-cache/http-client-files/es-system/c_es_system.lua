...
function showRespawnButton(victimDropItem)
	...
	bCallLSFD = guiCreateButton(x , y- 100, width, height,"LSFD",false)
	guiSetEnabled(bRespawn, false)
	guiSetEnabled(bCallLSFD, true)
	table.insert(cacheR,bRespawn)
	table.insert(cacheC,bCallLSFD)
	setTimer(function()
	...
	guiSetProperty(bCallLSFD,"size","3")
	guiSetFont(bCallLSFD,"sa-header")
	addEventHandler("onClientGUIClick", bRespawn, function () 
		if bRespawn then
			destroyElement(bRespawn)
			destroyElement(bCallLSFD)
			bRespawn = nil
			bCallLSFD = nil
			showCursor(false)
		...
		for k,v in ipairs(cacheR) do 
			destroyElement(v)
		end 
		for k,v in ipairs(cacheC) do 
			destroyElement(v)
		end 
		showCursor(false)
	end, false)
	addEventHandler("onClientGUIClick", bCallLSFD, function () 
			destroyElement(bCallLSFD)
			bCallLSFD = nil
			if isPedDead(localPlayer) then 
				triggerServerEvent("es-system:callLSFD", localPlayer, victimDropItem,localPlayer)
				outputChatBox("LSFD received you call and your postion.")
				for k,v in ipairs(cacheC) do 
					destroyElement(v)
				end 
				showCursor(false)
			else 
				outputChatBox("You are not dead",255)
				for k,v in ipairs(cacheR) do 
					destroyElement(v)
				end 
				for k,v in ipairs(cacheC) do 

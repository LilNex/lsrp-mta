...
function aPlayerScreenShot (player)
	if aScreenShotForm == nil then
		...
		aScreenShotForm		= guiCreateWindow	( x / 2 - 300, y / 2 - 125, 600, 250, "ادارة لقطات المسجلة", false )
		aScreenShotList		= guiCreateGridList	( 0.03, 0.08, 0.70, 0.90, true, aScreenShotForm )
		aScreenShotNew		= guiCreateButton	( 0.75, 0.08, 0.42, 0.09, "التقاط لقطة جديدة", true, aScreenShotForm )
		aScreenShotDelete	= guiCreateButton	( 0.75, 0.18, 0.42, 0.09, "حذف لقطة", true, aScreenShotForm )
		aScreenShotView		= guiCreateButton	( 0.75, 0.28, 0.42, 0.09, "مشاهدة لقطة", true, aScreenShotForm )
		aScreenShotRefresh	= guiCreateButton	( 0.75, 0.38, 0.42, 0.09, "تحديث لوحة", true, aScreenShotForm )
		aScreenShotClose	= guiCreateButton	( 0.75, 0.88, 0.42, 0.09, "اغلاق", true, aScreenShotForm )
		guiGridListAddColumn(aScreenShotList,"لاعب",0.31 )
		guiGridListAddColumn(aScreenShotList,"ادمين",0.31 )
		guiGridListAddColumn(aScreenShotList,"توقيت",0.27 )
	...
	if button == "left" then
		...
		elseif source == aScreenShotNew then
			if guiGridListGetSelectedItem(aTab1.PlayerList ) == -1 then
				aMessageBox("تنبيه","لم يتم تحديد أي لاعب !")
			else
...
addEventHandler("aClientScreenShot",resourceRoot,
	function (action,player,data,arg1,arg2,arg3)
		if action == "new" then
			...
			aScreenShotWindows[player].label = guiCreateLabel(0,0,1,1,"جار التحميل . . . ",true,aScreenShotWindows[player].window)
			aScreenShotWindows[player].button = guiCreateButton(0.93,0.95,0.6,0.4,"اغلاق",true,aScreenShotWindows[player].window)
		...
		elseif action == "list" then
			...
			elseif action == "minimized" then
				guiSetText(aScreenShotWindows[player].label,"يتم تصغير لقطة لهذا لاعب , المرجوا محاولة مرة اخرى")
			elseif action == "disabled" then
				guiSetText(aScreenShotWindows[player].label,"التقاط لقطات لهذه لاعب معطلة")
			elseif action == "quit" then
				guiSetText(aScreenShotWindows[player].label,"لاعب خرج")
			end
		end
	end
)

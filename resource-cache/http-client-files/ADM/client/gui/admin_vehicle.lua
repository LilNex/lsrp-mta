...
function aVehicleCustomize ( player )
	if ( aVehicleForm == nil ) then
		...
		aVehicleForm		= guiCreateWindow ( x / 2 - 300, y / 2 - 150, 600, 450, "ضبط المركبة", false )
		...
		for i = 1, 17 do
			if ( i ~= 12 ) then
				...
				guiGridListAddColumn( aVehicleUpgrades[c].list, "رقم تعريفي لترقية", 0.90 )
				guiSetVisible ( aVehicleUpgrades[c].list, false )
				c = c + 1
			end
		end

		aVehicleUpgradeAll	= guiCreateButton ( 0.04, 0.92, 0.15, 0.05, "تجديد الكل", true, aVehicleForm )
		aVehicleRemoveAll	= guiCreateButton ( 0.20, 0.92, 0.15, 0.05, "مسح الكل", true, aVehicleForm )
		aVehicleUpgrade	= guiCreateButton ( 0.375, 0.92, 0.20, 0.05, "تجديد المحدد", true, aVehicleForm )

		guiCreateStaticImage ( 0.60, 0.10, 0.002, 0.80, "client\\images\\dot.png", true, aVehicleForm )

					   guiCreateLabel ( 0.63, 0.10, 0.15, 0.05, "نوع الطلاء :", true, aVehicleForm )
		...
		aVehiclePaintjobSet	= guiCreateButton ( 0.90, 0.10, 0.07, 0.048, "تطبيق", true, aVehicleForm )
					   guiCreateLabel ( 0.63, 0.15, 0.15, 0.05, "لون المركبة :", true, aVehicleForm )
					   guiCreateLabel ( 0.63, 0.20, 0.15, 0.05, "اللون 1:", true, aVehicleForm )
					   guiCreateLabel ( 0.63, 0.25, 0.15, 0.05, "اللون 2:", true, aVehicleForm )
					   guiCreateLabel ( 0.63, 0.30, 0.15, 0.05, "اللون 3:", true, aVehicleForm )
					   guiCreateLabel ( 0.63, 0.35, 0.15, 0.05, "اللون 4:", true, aVehicleForm )
		...
		aVehicleColorSet	= guiCreateButton ( 0.84, 0.41, 0.14, 0.05, "وضع", true, aVehicleForm )
		guiCreateLabel ( 0.63, 0.5, 0.15, 0.05, "لون الاضواء :", true, aVehicleForm )
		aLightsColor = guiCreateEdit ( 0.79, 0.5, 0.13, 0.048, "#ffffff", true, aVehicleForm ) guiEditSetMaxLength ( aLightsColor, 7 )
		aLightsColorSet	= guiCreateButton ( 0.93, 0.5, 0.05, 0.05, "وضع", true, aVehicleForm )
		aVehicleUpgradeNames = guiCreateCheckBox ( 0.63, 0.7, 0.30, 0.04, "عرض اسامي الترقية", false, true, aVehicleForm )
					   if ( aGetSetting ( "aVehicleUpgradeNames" ) ) then guiCheckBoxSetSelected ( aVehicleUpgradeNames, true ) end
		aVehicleClose		= guiCreateButton ( 0.86, 0.92, 0.19, 0.05, "اغلاق", true, aVehicleForm )
		...
		guiCreateLabel ( 0.63, 0.61, 0.15, 0.05, "رقم اللوحة :", true, aVehicleForm )
		aLicensePlate = guiCreateEdit ( 0.79, 0.61, 0.13, 0.048, "", true, aVehicleForm ) guiEditSetMaxLength ( aLicensePlate, 8 )
		aLicensePlateSet = guiCreateButton ( 0.93, 0.61, 0.05, 0.05, "وضع", true, aVehicleForm )
	...
	if ( vehicle ) then
		...
		guiSetText ( aVehicleForm, "معلومات تعديل سيارة ("..tostring ( getVehicleName ( vehicle ) )..")" )

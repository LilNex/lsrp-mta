...
function aPlayerSkin ( player )
	if ( aSkinForm == nil ) then
		...
		aSkinForm		= guiCreateWindow ( x / 2 - 140, y / 2 - 125, 280, 250, "اختيار شخصية اللاعب", false )
		aSkinLabel		= guiCreateLabel ( 0.03, 0.09, 0.94, 0.07, "وضع رقم الشخصيه او اسمها", true, aSkinForm )
					  guiLabelSetHorizontalAlign ( aSkinLabel, "center" )
					  guiLabelSetColor ( aSkinLabel, 255, 0, 0 )
		aSkinGroups		= guiCreateCheckBox ( 0.03, 0.90, 0.70, 0.09, "اخراج المجموعات", false, true, aSkinForm )
		...
		aSkinList		= guiCreateGridList ( 0.03, 0.18, 0.70, 0.71, true, aSkinForm )
					  guiGridListAddColumn( aSkinList, "الرقم", 0.20 )
		...
		aSkinAccept		= guiCreateButton ( 0.75, 0.28, 0.27, 0.09, "اختيار", true, aSkinForm, "setskin" )
		aSkinCancel		= guiCreateButton ( 0.75, 0.88, 0.27, 0.09, "إلغاء", true, aSkinForm )
		...
		if ( source == aSkinAccept ) then
			if ( tonumber ( guiGetText ( aSkinID ) ) ) then
				...
				if ( guiGridListGetSelectedItem ( aSkinList ) ~= -1 ) then
					...
					aMessageBox ( "تحذير", "لم يتم اختيار أي لاعب / رقم الجلد!" )
				end
			end

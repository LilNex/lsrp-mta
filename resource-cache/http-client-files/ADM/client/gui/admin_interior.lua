...
function aPlayerInterior ( player )
	if ( aInteriorForm == nil ) then
		...
		aInteriorForm		= guiCreateWindow ( x / 2 - 110, y / 2 - 150, 220, 300, "وضع اللاعب في اي مبنى", false )
		aInteriorList		= guiCreateGridList ( 0.03, 0.08, 0.94, 0.73, true, aInteriorForm )
					   guiGridListAddColumn( aInteriorList, "الرقم", 0.2 )
					   guiGridListAddColumn( aInteriorList, "المبنى", 0.75 )
		aInteriorSelect		= guiCreateButton ( 0.03, 0.82, 0.94, 0.075, "اختيار", true, aInteriorForm )
		aInteriorCancel		= guiCreateButton ( 0.03, 0.90, 0.94, 0.075, "الغاء", true, aInteriorForm )
		...
		if ( node ) then
			...
			while ( xmlFindChild ( node, "interior", interiors ) ~= false ) do
				...
				guiGridListSetItemText ( aInteriorList, row, 1, xmlNodeGetAttribute ( interior, "الرقم" ), false, true )
				guiGridListSetItemText ( aInteriorList, row, 2, xmlNodeGetAttribute ( interior, "الرقم" ), false, false )

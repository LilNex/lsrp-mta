...
function showHouseMenu( absX, absY )
	...
	if interiorType>=0 and interiorType<3 then
		row.mailbox = rightclick:addRow("")
		addEventHandler("onClientGUIClick", row.mailbox, function(button)
			if button=="left" and not getElementData(getLocalPlayer(), "exclusiveGUI") and t == 1 then
...
function purchasePropertyGUI(interior, cost, isHouse, isRentable, neighborhood)
	...
	purchaseProperty.window[1] = guiCreateWindow(607, 396, 499, 199, "Purchase Property", false)
	...
	btnPosX = btnPosX + btnW + margin/2
	purchaseProperty.button[4] = guiCreateButton(btnPosX, 145, btnW, 43, "Preview Interior", false, purchaseProperty.window[1])
	guiSetProperty(purchaseProperty.button[4], "NormalTextColour", "FFAAAAAA")
	btnPosX = btnPosX + btnW + margin/2
	purchaseProperty.button[3] = guiCreateButton(btnPosX, 145, btnW, 43, "Close", false, purchaseProperty.window[1])
	...
	function()
		...
		if btnText == "Purchase using Cash" then
			triggerServerEvent("buypropertywithcash", getLocalPlayer(), getLocalPlayer(), interior, cost, isHouse, isRentable)
			closePropertyGUI()
		else
			btnTextSet = {"Purchase using Cash", "Purchase via Bank"}
	...
	function()
		...
		if btnText == "Purchase via Bank" then
			triggerServerEvent("buypropertywithbank", getLocalPlayer(), getLocalPlayer(), interior, cost, isHouse, isRentable)
			closePropertyGUI()
		else
			if isRentable then
	...
	function()	
		if getElementData(localPlayer, "adminjailed") then	
			-- هنا سوي للي تريده حطله باند للي يسويها اذا بدك ض		-- www.shoman-host.com 
			closePropertyGUI()	
			outputChatBox("[admin-jail]: Stay Here.  [(: By Shoman] شومان للشروحات يوتيوب")
			else

...
function strInfos()
	...
	for k,v in ipairs(getElementsByType("player")) do 
		local fId = tonumber(getElementData(v,"faction"))
		if fId then 
			if fId == 1 then 
				cLspd = cLspd + 1
	...
	str = string.format("LSPD : %s | LSFD : %s | RT : %s | LSSD : %s",cLspd,cLsfd,cRt,cLssd)
...
drawBackground = function ()
	...
	if g_currentHeight > SCOREBOARD_HEADER_HEIGHT then
		...
		dxDrawText ( str, SCOREBOARD_X+64, SCOREBOARD_Y + SCOREBOARD_HEADER_HEIGHT - 48)		
...
drawScoreboard = function ()
	...
	local serverName = getElementData ( g_scoreboardDummy, "serverName" ) or "Hata 404"
	...
	local strPlayerCount = "Players: " .. tostring(#g_players) .. "/100"



	-- We need to recalculate the left position, to make it not move when fading.

	local offset = SCOREBOARD_WIDTH - dxGetTextWidth ( strPlayerCount, 1, makeFont() or "default-bold" ) - 4
	...
	drawRowBounded ( "ID", "Name", "Hours",  "Ping",
	...
	local renderEntry = function ( player, seeUsername )
		...
		playerPing = tostring ( playerPing-20 )

-- Configuration
local background_color = tocolor( 0, 0, 0, 25 ) --tocolor( 0, 0, 0, 70 ) changed by HkHaythem
...
local splittableItem = {[30]=" gram(s)", [31]=" gram(s)", [32]=" gram(s)", [33]=" gram(s)", [34]=" gram(s)", [35]=" ml(s)", [36]=" tablet(s)", [37]=" gram(s)", [38]=" gram(s)", [39]=" gram(s)", [40]=" ml(s)", [41]=" tab(s)", [42]=" shroom(s)", [43]=" tablet(s)", [134] = " money",[300] = " dirtymoney" }
...
local actionIcons =
{
	[TAB_WALLET] = { -48, tocolor( 255, 255, 255, 50 ), "My Wallet" },
	[TAB_ITEMS] = { 48, tocolor( 255, 255, 255, 50 ), "My Items" },
	[TAB_KEYS] = { -203, tocolor( 255, 255, 255, 50 ), "My Keys" },
	[TAB_WEAPONS] = { -204, tocolor( 255, 255, 255, 50 ), "My Weapons" },
	
	[ACTION_DROP] = { -202, tocolor( 255, 127, 39, 63 ), "Drop Item", "Press CTRL while selecting an item to automatically drop it." },
	[ACTION_SHOW] = { -201, tocolor( 0, 255, 255, 63 ), "Show Item" },
...
local function tooltip( x, y, text, text2, to_delete )
	...
	local width = dxGetTextWidth( text, 1, "default-bold" ) + 20
	if text2 then
		width = math.max( width, dxGetTextWidth( text2, 1, "default-bold" ) + 20 )
		text = text .. "\n" .. text2
	end
					...
					if state == "up" and clickItemSlot then
						...
						elseif not clickItemSlot.group then
								...
								if getDistanceBetweenPoints3D( worldX, worldY, worldZ, getElementPosition( localPlayer ) ) < 10 then
									...
									if hoverElement == nil then
										if itemID > 0 then
											...
											elseif itemID == 273 then
											    exports.SCR_Info:addNotification("You can't drop this item ","info")
											else
							...
							elseif hoverAction == ACTION_DESTROY then
								...
								elseif itemID == 216 then
									outputChatBox("You can't destroy trash.")
								else
									triggerServerEvent( "destroyItem", localPlayer, itemSlot )
								end

...
function insuranceMainGUI(theTable) -- well it's not done -- ye just table.factionid and etc hey
	...
	local windowName = "[" ..theTable["name"].. "] Insurance System powered by Tree Technology"
	...
	local tclaims = 0
	local c = 0
	local fId = getElementData(getLocalPlayer(), "faction")
	...
	local leader = exports.factions:isPlayerFactionLeader(getLocalPlayer(), theTable["factionID"])
	...
	addEventHandler("onClientGUIClick", main.button[1], function () -- all those client click need a check  of source == the button. else you can click the window to trigger any of them
			...
			local result = triggerServerEvent("insurance:sql", resourceRoot, "news", getElementData(getLocalPlayer(), "faction"), news)
	...
	addEventHandler("onClientGUIClick", main.button[2], function ()
			...
			local result = triggerServerEvent("insurance:sql", resourceRoot, "subscription", getElementData(getLocalPlayer(), "faction"), subscription)
	...
	addEventHandler("onClientGUIClick", main.button[3], function ()
			...
			local result = triggerServerEvent("insurance:sql", resourceRoot, "gen_maxi", getElementData(getLocalPlayer(), "faction"), maxigen)
...
function quoteSimplicity(companyname)
	...
	addEventHandler("onClientGUIClick", simplicity.button[1], function ()
			...
			triggerServerEvent("insurance:calculate", localPlayer, tonumber(vinInput), getElementData(getLocalPlayer(), "faction"))
		end, false)
	...
	addEventHandler("onClientGUIClick", simplicity.button[3], function ()
			...
			triggerServerEvent("insurance:calculate", localPlayer, tonumber(vinInput), getElementData(getLocalPlayer(), "faction"), tonumber(charge))
		end, false)
...
--this doesn't sound right :D the output chatbox thing. let me think
			triggerServerEvent("insurance:finalpurchase", root, salesman, getLocalPlayer(), carid, protection, premium, getElementData(salesman, "faction") )
		end, false)
end
...
function manageInsurance(companyname)
	...
	guiGridListSetSortingEnabled(manage.gridlist[1], false)
	...
	for k,v in pairs(datatable) do
		local playerFaction = tonumber(getElementData(getLocalPlayer(), "faction"))
		if tonumber(v["insurancefaction"]) == playerFaction then
			local row = guiGridListAddRow(manage.gridlist[1])
			guiGridListSetItemText( manage.gridlist[1], row, manage.column[1], v["policyid"], false, false)
			guiGridListSetItemText( manage.gridlist[1], row, manage.column[2], v["customername"], false, false)
			guiGridListSetItemText( manage.gridlist[1], row, manage.column[3], v["vehicleid"], false, false)
			guiGridListSetItemText( manage.gridlist[1], row, manage.column[4], v["protection"], false, false)
			guiGridListSetItemText( manage.gridlist[1], row, manage.column[5], v["premium"], false, false)
			guiGridListSetItemText( manage.gridlist[1], row, manage.column[6], v["deductible"], false, false)
			guiGridListSetItemText( manage.gridlist[1], row, manage.column[7], v["date"], false, false)
			guiGridListSetItemText( manage.gridlist[1], row, manage.column[8], v["claims"], false, false)
			guiGridListSetItemText( manage.gridlist[1], row, manage.column[9], v["cashout"], false, false)
		end
	end
end

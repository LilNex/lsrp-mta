...
function vs1000_gui(element, data, shaderData, videoData)
	if GUIEditor.window[1] then destroyGui() end

	vs1000_element = element

	GUIEditor.window[1] = guiCreateWindow(223, 185, 324, 483, "ClubTec VS1000 Control Panel", false)
	...
	if hasDisc and videoData then
		...
		discMeta = "Created by "..tostring(videoData.creator)..", "..tostring(videoData.date)
	else
		discTitle = "No Disc"
		discMeta = " "
	end
	...
	if isClubtecGuy(localPlayer) then
		...
		GUIEditor.label[6] = guiCreateLabel(11, 404, 301, 17, "Texture", false, GUIEditor.window[1])
		...
		GUIEditor.edit[1] = guiCreateEdit(14, 420, 298, 25, tostring(data.texture), false, GUIEditor.window[1]) --w298
	...
	addEventHandler("onClientGUIClick", GUIEditor.button[1], function (button, state)
		...
		guiSetEnabled(GUIEditor.button[1], false)
		triggerServerEvent("clubtec:vs1000:ejectDisc", resourceRoot, vs1000_element)
	end, false)
	addEventHandler("onClientGUIClick", GUIEditor.button[2], function (button, state)
		...
		if isClubtecGuy(localPlayer) then
			...
			local newData = {brightness = newBrightness, scrollX = newScrollX, scrollY = newScrollY, xScale = newXScale, yScale = newYScale, rotAngle = newRotAngle, alpha = newAlpha, grayScale = newGray, redColor = newRed, grnColor = newGreen, bluColor = newBlue, xOffset = newXOffset, yOffset = newYOffset, texture = newTexture}

			triggerServerEvent("clubtec:vs1000:updateSettings", resourceRoot, vs1000_element, newData)
		end

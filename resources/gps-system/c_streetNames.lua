...
addEventHandler("onClientRender", getRootElement(),
	function()
		...
		if getDistanceBetweenPoints2D( islandx, islandy, x, y ) < 500 then
			setElementData(getLocalPlayer(), "speedo:street", "San Tortuguilla Island")
		elseif cacheX ~= x or cacheY ~= y or cacheZ ~= z then
			local node = findNodeClosestToPoint(vehicleNodes, x, y, z)
			setElementData(getLocalPlayer(), "speedo:street", node.streetname)
			cacheX = x
			cacheY = y
			cacheZ = z
		end

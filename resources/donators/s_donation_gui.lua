...
function showDonationGUI(donor)
	...
	if (logged==1) then
		...
		if (gameAccountID) then
			...
			if (gameAccountID > 0) then
				...
				if (mResult1) then
					...
					donPoints = tonumber(mResult1["credits"]) or 0
		...
		local mQuery2 = exports.mysql:query("SELECT `order_id`, `txn_id`, `payer_email`, `mc_gross`, `date`, `accounts`.`username` AS `donor`, (SELECT `username` FROM `accounts` WHERE `accounts`.`id`=`donates`.`donated_for`) AS `donated_for` FROM `donates` LEFT JOIN `accounts` ON `donates`.`donor` = `accounts`.`id` WHERE `donor`='"..gameAccountID.."' OR `donated_for`='"..gameAccountID.."' ORDER BY `date` DESC ")

		local donateHistory = {}

		while true do

			local row = exports.mysql:fetch_assoc(mQuery2)

			if not row then break end

			table.insert(donateHistory, row )

		end

		exports.mysql:free_result(mQuery2)
		...
		if exports.integration:isPlayerSeniorAdmin(client) then
			...
			local mQuery7 = exports.mysql:query("SELECT `name`, `cost`, `date`, (SELECT `username` FROM `accounts` WHERE `accounts`.`id`=`account`) AS `account` FROM `don_purchases` WHERE `cost` < 0 ORDER BY `date` DESC LIMIT 100") 
			...
			while true do
				...
				table.insert(globalPurchaseHistory, {row["name"], row["cost"], row["date"], row["account"] })
		...
		triggerClientEvent(client, "donation-system:GUI:open", client, obtainedTable, availableTable, donPoints, donateHistory, purchaseHistory, globalPurchaseHistory)			

	end

end
addCommandHandler("pp", showDonationGUI) 
...
function getOtherInfo(userID)
	...
	return exports.mysql:query_fetch_assoc("SELECT SUM(`mc_gross`), `payer_email` FROM `donates` WHERE `donor`='"..userID.."' ")
...
function activateDonationPerk(thePerk, ...)
	...
	if thePerk == 13 then -- GC transfer
		...
		if (data.total <= donPoints) then
			...
			if exports.mysql:query_free("UPDATE `accounts` SET `credits`=`credits`+'"..data.amount.."' WHERE `username`='"..data.target.."' ") then

...
function openDonationGUI(obtained1, available1, credits1, history1, purchased1, globalPurchaseHistory1)
	...
	if wDonation and isElement(wDonation) then
		...
		if exports.integration:isPlayerSeniorAdmin(localPlayer) then
		...
		col.name = guiGridListAddColumn(grid.availableItems,"Perk Name",0.6)
		col.duration = guiGridListAddColumn(grid.availableItems,"Duration",0.1)
		col.price = guiGridListAddColumn(grid.availableItems,"Cost",0.2)
		...
		col.a_name = guiGridListAddColumn(grid.activatedPerks,"Perk Name",0.6)
		...
		col.h_email = guiGridListAddColumn(grid.history,"Details",0.4)
...
function updateAvailablePerks()
	...
	for perkID, perkArr in ipairs(available) do
		if (perkArr[1] ~= nil) and (perkArr[2] ~= 0) then
			...
			if perkArr[4] == 14 then --max ints
				local nextIntCap = tonumber( getElementData(localPlayer, "maxinteriors") )+1
				guiGridListSetItemText(grid.availableItems, row, col.name, perkArr[1]..nextIntCap, false, false)
			elseif perkArr[4] == 15 then --max veh
				local currentMaxVehicles = tonumber( getElementData(localPlayer, "maxvehicles") )+1
				guiGridListSetItemText(grid.availableItems, row, col.name, perkArr[1]..currentMaxVehicles, false, false)
			else
		...
		function( button )
			if button == "left" then
				...
				if row ~= -1 and col ~= -1 then
					...
					local cCost = guiGridListGetItemText( grid.availableItems , row, 3 )
					local cID = guiGridListGetItemText( grid.availableItems , row, 4 )
					cID = tonumber(cID)
					if cID == 13 and gcTransferFee then
						showGcTransfer(cID, gcTransferFee)
					elseif tonumber(cID) == 18 or tonumber(cID) == 19 then
						showPhonePicker(cID)
					elseif cID == 20 or cID == 21 or cID == 22 or cID == 23 or cID == 28 or cID == 33 or cID == 35 or cID == 36 or cID == 38 then
						showInfoPanel(cID)
					elseif cID == 16 then
...
function updateObtainedPerks()
	...
	for perkID, perkTable in ipairs(obtained) do
		...
		if (perkArr[1] ~= nil) then
			...
			guiGridListSetItemText(grid.activatedPerks, row, col.a_id, perkArr[4] , false, true)
		end
	end
		...
		function( button )
			if button == "left" then
				...
				if row ~= -1 and col ~= -1 then
					...
					local aID = guiGridListGetItemText( grid.activatedPerks , row, 3 )
					if aID == "29" then
...
function updateMyHistory()
	...
	for i = 1, #history do
		...
		guiGridListSetItemText(grid.history, row, col.h_txn_id, history[i]["txn_id"] , false, false)
		guiGridListSetItemText(grid.history, row, col.h_email, "From "..(history[i]["donor"] or "Unknown").." ("..history[i]["payer_email"]..") to "..history[i]["donated_for"] , false, false)
		guiGridListSetItemText(grid.history, row, col.h_amount, "$"..history[i]["mc_gross"] , false, true)
...
function showPhonePicker(perkID)
	...
	gui["lblText2"] = guiCreateLabel(20, 45+15*2, windowWidth-40, 70, "By clicking on Purchase button, you agree that a refund is not possible. Thanks for donating!", false, wPhone)
...
function showUsernameChange(perkID)
	...
	guiUsername["lblText2"] = guiCreateLabel(20, 45+15*2, windowWidth-40, 70, "By clicking on Purchase button, you agree that a refund is not possible. Thanks for donating!", false, guiUsername.main)
...
function showGcTransfer(perkID, fee1)
	...
	guiGC["lblText2"] = guiCreateLabel(20, 45+15*6, windowWidth-40, 70, "By clicking on Transfer button, you agree that a refund is not possible. Thanks for donating!", false, guiGC.main)
...
function showConfirmSpend(perkName, perkDur, perkCost, perkID)
	...
	local btmText = "By clicking on Purchase button, you agree that a refund is not possible. Thanks for donating!"
...
function showLearnLanguageMenu(pID, pCost)
	...
	local content = "You can spend "..pCost.." on making your current character ("..tostring(getPlayerName(localPlayer)):gsub("_", " ")..") fully learning a selected language instantly. Please select language from the list below."
	...
	local btnW = (windowWidth-40)/2
	chatIcon["confirm"] = guiCreateButton(20, 120+length, btnW, 30, "Purchase ("..tostring(pCost)..")", false, chatIcon.wMain)
	addEventHandler( "onClientGUIClick", chatIcon["confirm"], function()
		if source == chatIcon["confirm"] then
			playSoundCreate()

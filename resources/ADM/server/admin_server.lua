...
function notifyPlayerLoggedIn(player)
	outputChatBox ( "Welcome admin to your panel !", player )
	local unread = 0
	for _, msg in ipairs ( aReports ) do
		unread = unread + ( msg.read and 0 or 1 )
	end
	if unread > 0 then
		outputChatBox( unread .. " رسالة تقرير غير مقروء" .. ( unread==1 and "" or "s" ), player, 255, 0, 0 )
	end
end
...
addCommandHandler ( "register", function ( player, command, arg1, arg2 )
	...
	if ( password ~= nil ) then
		if ( string.len ( password ) < 4 ) then
			outputChatBox ( "register: - كلمة المرور و الآمنة يجب ان تتكون من 4 احرف على", player, 255, 100, 70 )
		elseif ( addAccount ( username, password ) ) then
			outputChatBox ( "لقد سجلت بنجاح! اسم المستخدم: '"..username.."', Password: '"..password.."'(Remember it)", player, 255, 100, 70 )
		...
		outputChatBox ( "register: - بناء الجملة هو /register [<nick>] <password>", player, 255, 100, 70 )
	end
end )

...
function aSpectate ( player )
	if ( player == getLocalPlayer() ) then
		aMessageBox ( "خطأ", "لا يمكنك تصوير نفسك" )
		return
	end
...
function aSpectator.Initialize ()
	if ( aSpectator.Actions == nil ) then
		...
		aSpectator.Actions		= guiCreateWindow ( x - 190, y / 2 - 200, 160, 400, "الأوامر", false )
		aSpectator.Ban			= guiCreateButton ( 0.10, 0.09, 0.80, 0.05, "حظر", true, aSpectator.Actions )
		aSpectator.Kick			= guiCreateButton ( 0.10, 0.15, 0.80, 0.05, "طرد", true, aSpectator.Actions )
		aSpectator.Freeze			= guiCreateButton ( 0.10, 0.21, 0.80, 0.05, "تجميد", true, aSpectator.Actions )
		aSpectator.SetSkin		= guiCreateButton ( 0.10, 0.27, 0.80, 0.05, "تغيير الشخصيه", true, aSpectator.Actions )
		aSpectator.SetHealth		= guiCreateButton ( 0.10, 0.33, 0.80, 0.05, "إعطاء دم", true, aSpectator.Actions )
		aSpectator.SetArmour		= guiCreateButton ( 0.10, 0.39, 0.80, 0.05, "إعطاء درع", true, aSpectator.Actions )
		aSpectator.SetStats		= guiCreateButton ( 0.10, 0.45, 0.80, 0.05, "تعيين الإحصاءات", true, aSpectator.Actions )
		aSpectator.Slap			= guiCreateButton ( 0.10, 0.51, 0.80, 0.05, "سلب صحة"..aCurrentSlap.."-", true, aSpectator.Actions )
		...
		aSpectator.Skip			= guiCreateCheckBox ( 0.08, 0.85, 0.84, 0.04, "تجاوز لاعب فور موته", true, true, aSpectator.Actions )
					  		  guiCreateLabel ( 0.08, 0.89, 0.84, 0.04, "____________________", true, aSpectator.Actions )
		aSpectator.Back			= guiCreateButton ( 0.10, 0.93, 0.80, 0.05, "الخلف", true, aSpectator.Actions )

		aSpectator.Players		= guiCreateWindow ( 30, y / 2 - 200, 160, 400, "اللاعبين", false )
		...
		aSpectator.PlayerList		= guiCreateGridList ( 0.03, 0.07, 0.94, 0.92, true, aSpectator.Players )
					  		  guiGridListAddColumn( aSpectator.PlayerList, "اسم اللاعب", 0.85 )
		...
		aSpectator.Prev			= guiCreateButton ( x / 2 - 100, y - 50, 70, 30, "الخلف >", false )
		aSpectator.Next			= guiCreateButton ( x / 2 + 30,  y - 50, 70, 30, "< التالي", false )
...
function aSpectator.SwitchPlayer ( inc, arg, inc2 )
	...
	if ( #players <= 0 ) then
		aMessageBox ( "question", "لا أحد أن يشاهد ، خروج المشاهد ؟", "spectatorClose" )
		return
	end
	local current = 1
	...
	if ( next == current ) then
		aMessageBox ( "question", "لا أحد آخر بلائحة ، خروج المشاهد ؟", "spectatorClose" )
		return
	end
...
function aSpectator.Render ()
	...
	if ( not aSpectator.Spectating ) then
		dxDrawText ( "لا أحد للمشاهدة", sx - 170, 200, sx - 170, 200, tocolor ( 255, 0, 0, 255 ), 1 )
		return
	end
	...
	if ( not x ) then
		dxDrawText ( "حدث خطأ أثناء تلقي الإحداثيات", sx - 170, 200, sx - 170, 200, tocolor ( 255, 0, 0, 255 ), 1 )
		return
	end
	...
	local sx, sy = guiGetScreenSize ()
	dxDrawText ( "مراقبة لاعب : "..getPlayerName ( aSpectator.Spectating ), sx - 170, 200, sx - 170, 200, tocolor ( 255, 255, 255, 255 ), 1 )
	if ( _DEBUG ) then
		...
		if ( isCursorShowing () ) then
			dxDrawText ( "نصيحة : Mouse2 - لتغيير وضع الكاميرا", 20, sy - 50, 20, sy - 50, tocolor ( 255, 255, 255, 255 ), 1 )
		else
			dxDrawText ( "نصيحة : استخدم التمرير الماوس للتكبير / التصغير", 20, sy - 50, 20, sy - 50, tocolor ( 255, 255, 255, 255 ), 1 )
		end
	end
end

...
function aViewMessages ( player )
	if ( aMessagesForm == nil ) then
		...
		aMessagesForm	= guiCreateWindow ( x / 2 - 250, y / 2 - 125, 620, 250, "عرض الرسائل", false )

		aMessagesList		= guiCreateGridList ( 0.02, 0.09, 0.82, 0.85, true, aMessagesForm )
					   guiGridListAddColumn( aMessagesList, "#", 0.08 )
					   guiGridListAddColumn( aMessagesList, "العنوان", 0.46 )
					   guiGridListAddColumn( aMessagesList, "التاريخ", 0.23 )
					   guiGridListAddColumn( aMessagesList, "المرسل", 0.19 )
		aMessagesRead		= guiCreateButton ( 0.86, 0.15, 0.12, 0.09, "قراءه", true, aMessagesForm )
		aMessagesDelete		= guiCreateButton ( 0.86, 0.25, 0.12, 0.09, "حذف", true, aMessagesForm )
		aMessagesBanSerial	= guiCreateButton ( 0.86, 0.40, 0.12, 0.09, "منع السريال", true, aMessagesForm )
		aMessagesBanIP		= guiCreateButton ( 0.86, 0.50, 0.12, 0.09, "منع الاي بي", true, aMessagesForm )
		aMessagesRefresh	= guiCreateButton ( 0.86, 0.65, 0.12, 0.09, "تحديث", true, aMessagesForm )
		aMessagesClose		= guiCreateButton ( 0.86, 0.85, 0.12, 0.09, "إغلاق", true, aMessagesForm )
...
function aClientMessagesClick ( button )
	if ( button == "left" ) then
		...
		elseif ( source == aMessagesRead ) then
			local row = guiGridListGetSelectedItem ( aMessagesList )
			if ( row == -1 ) then aMessageBox ( "تحذير", "لم تحدد الرساله!", nil )
			else
				local id = guiGridListGetItemText ( aMessagesList, row, 1 )
				aViewMessage ( tonumber ( id ) )
			end
		elseif ( source == aMessagesDelete ) then
			local row = guiGridListGetSelectedItem ( aMessagesList )
			if ( row == -1 ) then aMessageBox ( "تحذير", "لم تحدد الرساله!" )
			else
				local id = guiGridListGetItemText ( aMessagesList, row, 1 )
		...
		elseif ( source == aMessagesBanSerial ) then
			...
			aInputBox ( "منع السريال", "منع السريال", data.suspect.serial, "banSerial", _, _, data.suspect.name, data.category )
		elseif ( source == aMessagesBanIP ) then
			local data = _messages[tonumber ( guiGridListGetItemText ( aMessagesList, guiGridListGetSelectedItem ( aMessagesList ), 1 ) )]
			aInputBox ( "منع الاي بي", "أدخل الاي بي منع", data.suspect.ip, "banIP", _, _, data.suspect.name, data.category )

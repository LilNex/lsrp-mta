...
function aInputBox ( title, message, default, action, vOne, vTwo, defaultNick, defaultReason )
	if ( aInputForm == nil ) then
		...
		aInputOk		= guiCreateButton ( 90, 80, 55, 17, "موافق", false, aInputForm )
		aInputCancel		= guiCreateButton ( 150, 80, 55, 17, "الغاء", false, aInputForm )
	...
	if not action or action ~= "banSerial" and action ~= "banIP" then
		...
		if not isElement(banSerialCombo) then
			banSerialCombo = guiCreateComboBox ( 180, 80, 120, 100, "للابد", false, aInputForm )
				guiComboBoxAddItem(banSerialCombo, "الدقائق")
				guiComboBoxAddItem(banSerialCombo, "الساعات")
				guiComboBoxAddItem(banSerialCombo, "الايام")
				guiComboBoxAddItem(banSerialCombo, "للابد")
		end
		...
		if not isElement(banDurationLbl) then
			banDurationLbl = guiCreateLabel ( 40, 83, 50, 15, ": المدة", false, aInputForm )
			guiLabelSetHorizontalAlign ( banDurationLbl, "center" )
		end

		if not isElement(banSerialLbl) then
			banSerialLbl = guiCreateLabel ( 20, 110, 270, 15, "ادخل اسم لاعب و سبب", false, aInputForm )
...
function aBanInputBox ( player )
	...
	if ( aBanInputForm == nil ) then
		...
		aBanInputRadio1Label	= guiCreateLabel ( 10, 20, 270, 15, "النوع:", false, aBanInputRadioSet1 )
		aBanInputRadio1A		= guiCreateRadioButton ( 50, 20, 50, 15, "اي بي", false, aBanInputRadioSet1 )
		aBanInputRadio1B		= guiCreateRadioButton ( 50, 35, 120, 15, "سريال", false, aBanInputRadioSet1 )
		...
		local yy = 5
		aBanInputRadio2Label	= guiCreateLabel ( 10, yy, 270, 15, "المده:", false, aBanInputRadioSet2 )
		aBanInputRadio2s = {}
		...
		aBanInputOk			= guiCreateButton ( 90, y, 55, 17, "موافق", false, aBanInputForm )
		aBanInputCancel		= guiCreateButton ( 150, y, 55, 17, "الغاء", false, aBanInputForm )
		y = y + 30
	...
	guiSetText ( aBanInputForm, "تبنيد اللاعب " .. string.gsub(getPlayerName(player), "#%x%x%x%x%x%x", "" ))
	guiSetText ( aBanInputLabel, "ضع سبب الباند" )
	aHideFloaters()
...
function aBanInputBoxFinish ()
	...
	if not bUseIP and not bUseSerial then
		aMessageBox ( "خطأ", "لم تحدد اكتب!" )

		return
	end

	if seconds == false then
		aMessageBox ( "خطأ", "لم تضع المدة!" )
		return
	end
...
function aMuteInputBox ( player )
	...
	if ( aMuteInputForm == nil ) then
		...
		local yy = 5
		aMuteInputRadio2Label	= guiCreateLabel ( 10, yy, 270, 15, "المدة:", false, aMuteInputRadioSet2 )
		aMuteInputRadio2s = {}
		...
		aMuteInputOk			= guiCreateButton ( 90, y, 55, 17, "موافق", false, aMuteInputForm )
		aMuteInputCancel		= guiCreateButton ( 150, y, 55, 17, "إلغاء", false, aMuteInputForm )
		y = y + 30
	...
	guiSetText ( aMuteInputForm, "منع اللاعب من التحدث " .. getPlayerName(player) )
	guiSetText ( aMuteInputLabel, "سبب منع اللاعب من التحدث" )
	aHideFloaters()
...
function aMuteInputBoxFinish ()
	...
	-- Validate settings
	if seconds == false then
		aMessageBox ( "خطأ", "لم تختار المده!" )
		return
	end

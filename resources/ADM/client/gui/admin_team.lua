...
function aPlayerTeam ( player )
	if ( aTeamForm == nil ) then
		...
		aTeamForm		= guiCreateWindow ( x / 2 - 150, y / 2 - 125, 300, 250, "إدارة الفريق", false )
		aTeamLabel		= guiCreateLabel ( 0.03, 0.09, 0.94, 0.07, "اختيار فريق او إنشاء فريق جديد", true, aTeamForm )
		...
		aTeamList		= guiCreateGridList ( 0.03, 0.18, 0.50, 0.71, true, aTeamForm )
					  guiGridListAddColumn( aTeamList, "الفرق", 0.85 )
		aTeamRefresh		= guiCreateButton ( 0.03, 0.90, 0.50, 0.08, "تحديث", true, aTeamForm )
		aTeamNew		= guiCreateButton ( 0.55, 0.18, 0.42, 0.09, "إنشاء فريق", true, aTeamForm, "createteam" )
		aTeamDelete		= guiCreateButton ( 0.55, 0.28, 0.42, 0.09, "حذف فريق", true, aTeamForm, "destroyteam" )
		aTeamShowColor	= guiCreateCheckBox ( 0.55, 0.38, 0.42, 0.09, "إظهار لون الفريق", true, true, aTeamForm )
		aTeamNameLabel	= guiCreateLabel 	( 0.55, 0.19, 0.42, 0.07, "اسم الفريق:", true, aTeamForm )
		aTeamColor		= guiCreateLabel 	( 0.55, 0.37, 0.42, 0.11, "اللون:", true, aTeamForm )
		...
		aTeamCreate		= guiCreateButton ( 0.55, 0.73, 0.20, 0.09, "إنشاء", true, aTeamForm, "createteam" )
		aTeamCancel		= guiCreateButton ( 0.77, 0.73, 0.20, 0.09, "إلغاء", true, aTeamForm )
		aTeamAccept		= guiCreateButton ( 0.55, 0.88, 0.20, 0.09, "أختيار", true, aTeamForm )
		aTeamClose		= guiCreateButton ( 0.77, 0.88, 0.20, 0.09, "إغلاق", true, aTeamForm )
		aTeamRemove		= guiCreateButton ( 0.55, 0.78, 0.42, 0.09, "إزالة لاعب من الفريق", true, aTeamForm )
...
function aClientTeamClick ( button )
	if ( button == "left" ) then
		...
		elseif ( source == aTeamDelete ) then
			if ( guiGridListGetSelectedItem ( aTeamList ) == -1 ) then
				aMessageBox ( "تنبيه", "لم يتم اختيار أي فريق!" )
			else
				local team = guiGridListGetItemText ( aTeamList, guiGridListGetSelectedItem ( aTeamList ), 1 )
				aMessageBox ( "تنبيه", "هل انت متأكد من الحذف \""..team.."\"?", "deleteTeam", team )
			end
		...
		elseif ( source == aTeamCreate ) then
			...
			if ( ( team == nil ) or ( team == false ) or ( team == "" ) ) then
				aMessageBox ( "تنبيه", "أدخل اسم الفريق!" )
			elseif ( getTeamFromName ( team ) ) then
				aMessageBox ( "المعذرة", "يوجد بالفعل فريق بهذا الاسم" )
			else
		...
		elseif ( source == aTeamRemove ) then
			if getPlayerTeam( aTeamSelect ) then
				...
				aMessageBox( "تنبيه", "هذا اللاعب ليس في فريق!")
			end
		end
	end
end

...
function aReport ( )
	if ( aReportForm == nil ) then
		...
		aReportForm		= guiCreateWindow ( x / 2 - 150, y / 2 - 170, 300, 340, "الاتصال بالادمن", false )
					   guiCreateLabel ( 0.05, 0.11, 0.20, 0.07, "القسم:", true, aReportForm )
					   guiCreateLabel ( 0.05, 0.19, 0.20, 0.07, "العنوان:", true, aReportForm )
					   guiCreateLabel ( 0.05, 0.34, 0.20, 0.07, "الرساله:", true, aReportForm )
		aReportLblPlayer = guiCreateLabel ( 0.05, 0.27, 0.20, 0.07, "لاعب:", true, aReportForm )
		aReportBtnPlayer = guiCreateButton ( 0.75, 0.27, 0.20, 0.07, "تحديد", true, aReportForm )
		aReportCategory	= guiCreateEdit ( 0.30, 0.10, 0.65, 0.07, "سؤال", true, aReportForm )
		...
		aReportAccept	= guiCreateButton ( 0.40, 0.88, 0.25, 0.09, "إرسال", true, aReportForm )
		aReportCancel	= guiCreateButton ( 0.70, 0.88, 0.25, 0.09, "إلغاء", true, aReportForm )
...
function aReportSelectPlayer ( )
	if ( aSelectPlayer == nil ) then
		local x, y = guiGetScreenSize ( )
		aSelectPlayer = guiCreateWindow ( x / 2 - 155, y / 2 - 250, 310, 500, "تحديد لاعب", false)
		...
		guiGridListAddColumn ( playerList, "اسم لاعب", 0.85 )
		for _, player in pairs (getElementsByType("player")) do
			guiGridListSetItemText(playerList, guiGridListAddRow(playerList), 1, getPlayerName(player), false, false)
		end
		local btnSelectPlayer = guiCreateButton(0.57, 0.93, 0.33, 0.05, "تحديد", true, aSelectPlayer)
		...
		local btnClose = guiCreateButton(0.10, 0.93, 0.33, 0.05, "اغلاق", true, aSelectPlayer)
	...
	if ( button == "left" ) then
		if ( source == aReportAccept ) then
			if ( ( string.len ( guiGetText ( aReportSubject ) ) < 1 ) or ( string.len ( guiGetText ( aReportMessage ) ) < 5 ) ) then
				aMessageBox ( "تنبيه", "محتوى الرسالة فارغ" )
			else
				local tableOut = {}
				...
				aMessageBox ( "تنبيه", "تم ارسال رسالة بنجاح" )

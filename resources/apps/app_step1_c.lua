...
function startStep11(quests1, retest, remainHours, remainMinutes, remainSeconds)
	...
	local margin = 30
	guiStep11.intro = guiCreateLabel(startX, startY, width, line*2, " قبل ان تصبح مؤهلا للعب في هذا السيرفر يجب عليك اجتياز هذا الاختبار القصير. (Section 1/2)", false )
	...
	guiStep11.q43 = guiCreateLabel(0, 0, panelW, line, (quests[4].question or "-"), false, guiStep11.q4 )
	guiSetFont(guiStep11.q43, "default-bold-small")
	...
	guiStep11.q53 = guiCreateLabel(0, 0, panelW, line, (quests[5].question or "-"), false, guiStep11.q5 )
	guiSetFont(guiStep11.q53, "default-bold-small")
	...
	guiStep11.q63 = guiCreateLabel(0, 0, panelW, line, (quests[6].question or "-"), false, guiStep11.q6 )
	guiSetFont(guiStep11.q63, "default-bold-small")
	...
	if text1 then 
		guiSetEnabled(guiStep11.next, false)
		guiSetText(guiStep11.intro, "لقد تم اعاده ارسالك الى تقديم الدخول "..text1.." لكي ترا قوانين السيرفر وتعد محاوله التقديم.")
...
function clickOnApplication()
	if source == guiStep11.next then
		...
		if incorrects > 0 then
			...
			guiSetText(guiStep11.intro, "تهانينا! لقد تم اجتياز هذا الاختبار والان الى الاختبار الاخير..")

	...
	if checkingItem and checkingPrice and (tonumber(checkingPrice) == tonumber(itemPrice) ) then
		...
		if success then
			...
			if not payByBank then
				...
				if playerGender == 0 then
					triggerEvent('sendAme', thePlayer, "takes out a couple of dollar notes from his wallet, hands it over to "..pedName)
				else					
					triggerEvent('sendAme', thePlayer, "takes out a couple of dollar notes from her wallet, hands it over to "..pedName)
				end
				...
				playBuySound(shopElement)
				if playerGender == 0 then
					triggerEvent('sendAme', thePlayer, "takes out a credit card from his wallet, hands it over to "..pedName)
				else
					triggerEvent('sendAme', thePlayer, "takes out a credit card from her wallet, hands it over to "..pedName)
				end
...
function collectMoney(thePlayer, shopElement)
	...
	local row1 = mysql:query_fetch_assoc("SELECT `sIncome` FROM `shops` WHERE `id` = '" .. mysql:escape_string(getElementData(shopElement, "dbid")) .. "' " ) or false
	if not row1 or not tonumber(row1.sIncome) or not (sIncome == tonumber(row1.sIncome)) then
		return false
	else
		if sIncome ~= tonumber(row1.sIncome) then
			return false
		end
	end
	...
	if profit >= 0 then
		if sIncome > 0 then
			...
			exports.global:giveMoney(thePlayer, sIncome)
			return true
		end
...
function storeKeeperSay(thePlayer, content, pedName)
	...
	exports.global:sendLocalText(thePlayer, "["..languagename.."] "..tostring(pedName).." says: "..content, 255, 255, 255, 30, {}, true)
end

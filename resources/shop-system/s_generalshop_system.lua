...
function createShopKeeper(x,y,z,interior,dimension,id,shoptype,rotation, skin, sPendingWage, sIncome, sCapacity, currentCap, sSales, pedName, sContactInfo, faction_belong, faction_access) 
	...
	setElementData(ped, "ped:type", "shop", true)
...
addEventHandler( "shop:buy", resourceRoot, function( index )
	...
	if not shop then
		...
		outputChatBox("Error " .. error .. "-NE, report at F2.", client, 255, 0, 0 )
	...
	if not item then
		...
		outputChatBox("Error " .. error .. "-NEI-" .. index .. ", report at F2", client, 255, 0, 0 )
	...
	if itemID == 270 or itemID == 271 or itemID == 272 then
	...
	if wonTheLottery or exports.global:giveItem( client, itemID, itemValue ) then
		...
		if itemID == 2 then
			...
			outputChatBox("You should buy a Sim to use this phone.", client, 255, 194, 14 )

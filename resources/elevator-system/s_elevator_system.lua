	...
	if ( ( vehicle and elevatorStatus[INTERIOR_TYPE]  ~= 0 and getVehicleOccupant( vehicle ) == player ) or not vehicle ) then
		...
		elseif isElement(player) then
			if movingInSameInt then
				...
				exports["interior-system"]:setPlayerInsideInterior(interiorElement, player, otherCP,movingInSameInt)
			end
			return true
		else
	...
	if isElement(vehicle) then
		...
		for i = 0, getVehicleMaxPassengers( vehicle ) do
			...
			if player then
				...
				--exports["interior-system"]:setPlayerInsideInterior(interiorElement, player, other )

		...
		if not (targetPlayer) or not (...) then
			...
			if not (targetPlayer) then
				...
				elseif (logged==1) then
					...
					mysql:query_free("UPDATE `accounts` SET `appstate`='0', `appreason`='" .. mysql:escape_string(reason) .. "', appdatetime = NOW() + INTERVAL 1 DAY, `monitored` = 'Forceapped for " .. mysql:escape_string(reason) .. "' WHERE `id`='" .. mysql:escape_string(id) .. "'")
					...
					mysql:query_free("UPDATE `accounts` SET `appstate`='0' WHERE `id`='" .. id .. "' ")
...
function unforceApplication(thePlayer, commandName, targetPlayer) --Maxime
	if (exports.integration:isPlayerTrialAdmin(thePlayer) or exports.integration:isPlayerSupporter(thePlayer)) then
		if not (targetPlayer) then
			...
			local limit = 10
			local sql = mysql:query("SELECT `id`, `username` FROM `accounts` WHERE `appstate` != 3 AND `username` LIKE '%"..exports.global:toSQL(targetPlayer).."%' LIMIT "..limit)
			local userInfo = {}
			while true do
			...
			mysql:query_free("UPDATE `accounts` SET `appstate`='3', `appreason`=NULL, appdatetime = NULL, `monitored` = 'Has been unforceapp-ed by " .. mysql:escape_string(staffName) .. "' WHERE `id`='" .. mysql:escape_string(userid) .. "'")

...
function addTexture(id, texName, object, loadimg, shaderData)
	...
	if allExist then
		...
		local animData = {id = id, textures = textureTable, shader = shadData.shader, texname = texName, currentFrame = 1, lastTick = 0 }
...
function removeAllTextures()
	for k,v in ipairs(animQueue) do
		...
		for k3,v3 in ipairs(v.textures) do
			destroyElement(v3)
		end
		table.remove(animQueue, k)
	end
	animQueue = {}
	loaded = {}
	disabledFakevids = {}
...
function renderAnims()
	...
	for k,v in ipairs(animQueue) do
		...
		if (getTickCount() - lastTick) > speed then
...
addEventHandler('fakevideo:updateShader', root, function(texName, object, shaderData, newTexName, newObject)
	...
	if shader then
		if(texName ~= newTexName or object ~= newObject) then --if the texture or object to replace changed
			--TODO: if object changed
			engineRemoveShaderFromWorldTexture(shader, texName, object)
			local newLoadedData = { id = loadedData.id, texture = loadedData.texture, shader = shader, object = newObject }
			...
			if animQueueNum and animDataOld then
				local animData = animDataOld -- = {id = animDataOld.id, textures = animDataOld.textures, shader = shader, texname = newTexName, currentFrame = animDataOld.currentFrame, lastTick = animDataOld.lastTick }

...
function fetchBuriedCharacters()
	local preparedQ = "SELECT `charactername`, DATE_FORMAT(`death_date`,'%b. %d. %Y') AS `dead`, `age`, `day` FROM `characters` WHERE `cked`='2' AND `death_date` IS NOT NULL ORDER BY `death_date` DESC LIMIT "..(#tombs).."; "

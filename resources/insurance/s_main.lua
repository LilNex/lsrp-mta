...
function openGUI(thePlayer)
	...
	if factionID > 0 and factionstable[factionID] then
		updateClientCache(thePlayer, {datatable, factionstable})
		triggerClientEvent(thePlayer, "insurance:mainGUI", resourceRoot, factionstable[factionID]) -- ya , but this function can bring with it the tables to client too
	end
end
...
function getVehicleData(id)
	...
	if theVehicle then -- if vehicle element is found
		...
		local ownerName = exports['cache']:getCharacterName(owner) or "REF. SUP."
...
function finalizePurchase(salesman, customer, carid, protection, premium, factionid)
	if isElement(customer) and carid and protection and premium and factionid then
		...
		if theVehicle then
			...
			exports.anticheat:setEld(theVehicle, "insurance:fee", tonumber(premium), false, true)
			exports.anticheat:setEld(theVehicle, "insurance:faction", tonumber(factionid), false, true)
			local realTime = getRealTime()
			local date = string.format("%04d/%02d/%02d", realTime.year + 1900, realTime.month + 1, realTime.monthday )
			local q = exports.mysql:query("INSERT INTO `owl_mta`.`insurance_data` (`customername`, `vehicleid`, `protection`, `deductible`, `date`, `claims`, `cashout`, `premium`, `insurancefaction`) VALUES ('"..string.gsub(getPlayerName(customer), "_", " ").."', '"..exports.mysql:escape_string(tonumber(carid)).."', '"..exports.mysql:escape_string(protection).."', '0', '"..date.."', '0', '0', '"..exports.mysql:escape_string(tonumber(premium)).."', '"..exports.mysql:escape_string(tonumber(factionid)).."')")
...
function cancelPolicy(vehicleID) -- why don't you put it a localplayer, but resourceroot, cuz clearly localplayer is the one that clicked the GUI
	if vehicleID then
		vehicleID = tonumber(vehicleID)
		if tonumber(datatable[vehicleID]["vehicleid"]) == vehicleID then -- attempt to index field ( a nil value )
			...
			local success2 = exports.mysql:query_free("DELETE FROM `owl_mta`.`insurance_data` WHERE `policyid`='"..exports.mysql:escape_string(policyid).."'")
			local theVehicle = exports.pool:getElement("vehicle", tonumber(vehicleID))
			local success3 = "false"
			if theVehicle then
				exports.anticheat:setEld(theVehicle, "insurance:fee", 0, false, true)
				exports.anticheat:setEld(theVehicle, "insurance:faction", 0, false, true)
				success3 =  "true"
			end
			datatable[vehicleID] = nil
			updateClientCache(source, {datatable, factionstable})

			outputChatBox("SYSTEM... [database2]="..tostring(success2).." | [policy number]="..success3, client)
			if success and success2 and success3 == "true" then
				outputChatBox("The policy id #"..policyid.." has been cancelled.", client)
			end
		end
	end
end

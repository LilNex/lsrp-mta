...
function showBankUI(isInFaction, isFactionLeader, factionBalance, depositable, limit, withdrawable, factionType)
	if not (wBank) then
		...
		wBank = guiCreateWindow(x, y, width, height, "Bank - Los Santos Roleplay", false)
		guiWindowSetSizable(wBank, false)
		setTimer(function()--shoman - لا تسرق يا منوب ^_^ خلص اسرقها حلال عليك
		...
		if (isInFaction) and (isFactionLeader) and not (bannedTypes[factionType]) and t == 1 then

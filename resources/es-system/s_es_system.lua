	...
	if getElementData(source, "dbid") then
		...
		elseif getElementData(source, "jailed") then
			...
			-- if killer and (getElementData(killer, "hoursplayed" ) >= 20) then
				-- victimDropItem = true
			-- end
			changeDeathViewTimer = setTimer(changeDeathView, 50, 0, source, victimDropItem)
			setTimer( function(source,victimDropItem)
				triggerClientEvent(source,"es-system:showRespawnButton",source, victimDropItem)
			end,
			5000,1,source,victimDropItem)
			...
			outputChatBox("You can respawn after 3 minutes.", source)
			-- backupPlayer(source)
...
function respawnPlayer(thePlayer, victimDropItem)
	if (isElement(thePlayer)) then
		...
		-- take dirtymoney
		local dMoney = exports.global:hasDirtyMoney(thePlayer,0)
		if dMoney > 0 then 
			outputChatBox("LSFD Employee: We handed your money over to the LSPD.",thePlayer, 255, 194, 14)
			exports.logs:dbLog(thePlayer, 34, thePlayer, " lost "..tostring(dMoney).."$ item "..tostring(i))
			exports.global:giveDirtyMoney(thePlayer,0)

			exports["item-system"]:takeItem(thePlayer,300)
		...
		if not victimDropItem then
			...
			for itemSlot, itemCheck in ipairs(items) do
				if itemCheck[1] == 115 then -- Weapon
					...
					if (((weapon >= 16 and weapon <= 40 and (gunlicense == 0 and gunlicense2 == 0) or not exports["weapon-system"]:isWeaponCCWP(thePlayer, itemCheck[1], itemCheck[2])) or (weapon == 29 or weapon == 30 or weapon == 32 or weapon ==31 or weapon == 34) and (gunlicense2 == 0)) and (not exports.factions:isPlayerInFaction(thePlayer, 2))) or (weapon >= 35 and weapon <= 38) then
				...
				elseif itemCheck[1] == 116 then
					...
					if cart_id and gunlicense == 0 and gunlicense2 == 0 then
						if exports['item-system']:takeItemFromSlot(thePlayer, itemSlot-cor) then
							cor = cor+1
							local pack = exports["weapon-system"]:getAmmo(cart_id)
...
function recoveryPlayer(thePlayer, commandName, targetPlayer, duration)
	if not (targetPlayer) or not tonumber(duration) or (tonumber(duration) or 0) <= 0 then
		...
		if targetPlayer then
			...
			if (logged==1) then
				if getElementData(getPlayerTeam(thePlayer),"type") or (exports.integration:isPlayerTrialAdmin(thePlayer) == true) then
...
function prescribe(thePlayer, commandName, ...)
	if getElementData(getPlayerTeam(thePlayer),"type") == 4 then
		if not (...) then

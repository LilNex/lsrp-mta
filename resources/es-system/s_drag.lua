...
function acceptCarry(thePlayer)
	...
	if requestPlayer then
		...
		local distance = getDistanceBetweenPoints3D(x, y, z+0.5, tx, ty, tz)
		if distance < 3 then
...
function allowCarry(thePlayer, carryPlayer)
	...
	attachElements(carryPlayer, thePlayer, 0.2, 0.45, 1.3)
	...
	setElementData(thePlayer, "carriedplayer", carryPlayer)
...
function stopEnter(theVehicle, seat, jacked)
	if getElementData(source, "carryplayer") == true then
		if getElementData(thePlayer, "carriedplayer") then 
			local cp = getElementData(thePlayer, "carriedplayer")
			warpPedIntoVehicle ( cp, theVehicle ,4)  
		end
 	-- 	cancelEvent()

...
function reloadCharacters(player)
...
function spawnCharacter(characterID, remoteAccountID, theAdmin, targetAccountName, location)
        ...
        if characterData then
                ...
                charProp = mysql:query_fetch_assoc("SELECT * FROM `interiors` WHERE `owner`='" .. tostring(characterID) .. "'")

                hourLogin = tonumber(string.sub(tostring(characterData["lastlogin"]),11,13))
                DateNow = getRealTime()
                comp = tonumber(DateNow.hour) - hourLogin 
                locationToSpawn = {}
                if comp < 1 and comp >= 0 then
                        ...
                        if charProp then
                                locationToSpawn.x = tonumber(charProp["interiorx"])
                                locationToSpawn.y = tonumber(charProp["interiory"])
                                locationToSpawn.z = tonumber(charProp["interiorz"])
                                locationToSpawn.rot = tonumber(charProp["angle"])
                                locationToSpawn.int = tonumber(charProp["interior"])
                                locationToSpawn.dim = tonumber(charProp["id"])
        
                        else
                                locationToSpawn.x = 1642.7421875
                                locationToSpawn.y = -2332.4501953125
                                locationToSpawn.z = 13.546875
                                locationToSpawn.rot = 360
                                locationToSpawn.int = 0
                                locationToSpawn.dim = 0
                        end
                end
                
                local dirtyMoney = exports.global:hasDirtyMoney(client , 0) or 0
                setElementData(client,"dirtymoney", dirtyMoney )
	  ...
	  setElementData(client, "recharge", tonumber(characterData["qrecharge"]))
...
function Characters_onCharacterChange()
        ...
        setElementData(client, "recharge")
mysql:query_free( "UPDATE characters SET qrecharge = " .. recharge .. " WHERE id = " .. getElementData( client, "dbid" ) )
...
local recharge = getElementData(client, "recharge") or 100
mysql:query_free( "UPDATE characters SET qrecharge = " .. recharge .. " WHERE id = " .. getElementData( client, "dbid" ) )
setElementAlpha(client, 255)
end
...
function GrabDataEmailDate(client,clientAccountID)
local clientAccountID = getElementDataEx(client, "account:id") or -1
local accountData = mysql:Query("SELECT * FROM `accounts` WHERE `id` = (?);", clientAccountID)
	local emailAddress = tostring(accountData[1].email)
	exports.anticheat:setElementDataEx(client, "temp:account:email", emailAddress, true)
end

function saverecharge(thePlayer,recharge)
local recharge = getElementData(thePlayer, "recharge") or 100
mysql:query_free( "UPDATE characters SET qrecharge = " .. recharge .. " WHERE id = " .. getElementData( thePlayer, "dbid" ) )
end
addEventHandler ("onPlayerQuit", root, saverecharge)
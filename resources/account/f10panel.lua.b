--- Added by Ramsey
-- F10 Settings Reworked by Ramsey
local enabledF10 = false
local sx, sy = guiGetScreenSize()
function enabledF10Func()
	if (enabledF10 == false and getElementData(localPlayer,"loggedin")) then
		enabledF10 = true
		addEventHandler("onClientRender",getRootElement(),enabledF10Render)
		showCursor(true)
		showChat(false)
	
		getElementData(localPlayer, "hudVisible")
		toggleControl("change_camera",false)
	else
		enabledF10 = false
		removeEventHandler("onClientRender",getRootElement(),enabledF10Render)
		showCursor(false)
		showChat(true)
...
bindKey("F10", "down", enabledF10Func)
color = "#FFFFFF"
text = "   Change Character"
function enabledF10Render()
	--black blur
	exports.r_interface:blurCreate()
	--dxDrawRectangle(0, 0, sx,sy, tocolor(0,0,0,100), true)
	--                                  en   boy
	dxDrawRectangle(sx/2-210, sy/2-150, 400, 400, tocolor(0,0,0,200))
	-- üst bilgi & label
	dxDrawRectangle(sx/2-210, sy/2-150, 400, 50, tocolor(0,0,0,120))
	dxDrawText("Los Santos Roleplay v1.1 ", sx/2-140, sy/2-138, 20, 20, tocolor(255,255,255,255), 1.5, "default-bold")
	--    Change Character button
	dxDrawRectangle(sx/2-210, sy/2-90, 400, 30, tocolor(0,0,0,120))
	dxDrawText(color..""..text, sx/2-90, sy/2-84, 20, 20, tocolor(255,255,255,255), 1.2, "default-bold","left","top",false,false,true,true)
	...
	if(isCursorShowing()) then
		...
		cursorX, cursorY = cursorX*XY[1], cursorY*XY[2]
			if(dobozbaVan(sx/2-210, sy/2-90, 400, 30, cursorX, cursorY)) then
				color = "#B04AEF"
				--options_logOut()		
			else
				color = "#FFFFFF"
...
addEventHandler("onClientClick", root,
	function(button, state, absoluteX, absoluteY, worldX, worldY, worldZ, clickedElement)	
		...
		if button == "left" and state == "down" then				
			if dobozbaVan(sx/2-210, sy/2-90, 400, 30, absoluteX, absoluteY) then
...
function dobozba(dX, dY, dSZ, dM, eX, eY)
	if(eX >= dX and eX <= dX+dSZ and eY >= dY and eY <= dY+dM) then
		return true
	else
		return false
...
-- bindkey kısmı devre dışı bıraklına alt taraf
...
function options_showmenu()
	...
	bStatistics = guiCreateButton(margin, wHeight, 230, 30, "Karakter Bilgileri", false, wOptions)
	...
	if getResourceFromName("donators") then
		bStore = guiCreateButton(margin, wHeight, 230, 30, "Vıp Paneli", false, wOptions)
		--addEventHandler("onClientGUIClick", bGraphicsSettings, options_opengraphicsmenu, false)
	...
	if getResourceFromName("admin-system") and exports['admin-system']:canPlayerAccessStaffManager(localPlayer) then
		bStaffManager = guiCreateButton(margin, wHeight, 230, 30, "Admin Paneli", false, wOptions)
	...
	if getResourceFromName("interior-system") and getResourceFromName("interior-manager") and exports.integration:isPlayerAdmin( localPlayer ) then
		bInteriorManager = guiCreateButton(margin, wHeight, 230, 30, "Ev ayarlama", false, wOptions)
	...
	if getResourceFromName("apps") and exports.integration:isPlayerTrialAdmin(localPlayer) or exports.integration:isPlayerSupporter(localPlayer) then
		bApplicationManager = guiCreateButton(margin, wHeight, 230, 30, "Uygulama Yöneticisi", false, wOptions)
	...
	if getResourceFromName("carradio") then
		bRadioManager = guiCreateButton(margin, wHeight, 230, 30, "Radyo İstasyonu Müdürü", false, wOptions)
	...
	if getResourceFromName("vehicle-system") and getResourceFromName("vehicle-manager") then
	...
	if getResourceFromName("announcement") and exports.announcement:canPlayerAccessMotdManager(localPlayer) then
		bmotd = guiCreateButton(margin, wHeight, 230, 30, "MOTD Ayarlama", false, wOptions)
	...
	if getResourceFromName("help") then
		bHelp = guiCreateButton(margin, wHeight, 230, 30, "Yardım", false, wOptions)
	...
	bLogout = guiCreateButton(margin, wHeight, 230, 30, "Karakter Çıkış", false, wOptions)
	...
	bClose = guiCreateButton(margin, wHeight, 230, 30, "Çıkış", false, wOptions)
...
function options_opengraphicsmenu()
	...
	cMotionBlur = guiCreateCheckBox(10, 25, 180, 17, "Hareket bulanıklığını etkinleştirme", false, false, wGraphicsMenu)
	addEventHandler("onClientGUIClick", cMotionBlur, options_updateGameConfig)
	-----------
	cSkyClouds = guiCreateCheckBox(10, 45, 180, 17, "Gökyüzü bulutları etkinleştirme", false, false, wGraphicsMenu)
	addEventHandler("onClientGUIClick", cSkyClouds, options_updateGameConfig)
	------------
	cStreamingAudio = guiCreateCheckBox(10, 65, 180, 17, "Ses akışı etkinleştirme", false, false, wGraphicsMenu)
	addEventHandler("onClientGUIClick", cStreamingAudio, options_updateGameConfig)

	bOverlayDescription = guiCreateButton ( 10, 85, 180, 17*2, "Chatbox istemci günlüğünü etkinleştirme", false, wGraphicsMenu )
	...
	cBubblesEnabled = guiCreateCheckBox(10, 180, 180, 17, "Sohbet kabarcıklar etkinleştirme", false, false, wGraphicsMenu)
	addEventHandler("onClientGUIClick", cBubblesEnabled, options_updateGameConfig)

	cIconsEnabled = guiCreateCheckBox(10, 200, 180, 17, "Yazarken Çıkan Simge", false, false, wGraphicsMenu)
	...
	cEnableRShaders = guiCreateCheckBox(10, 240, 180, 17, "Radar shader etkinleştirme", false, false, wGraphicsMenu)
	addEventHandler("onClientGUIClick", cEnableRShaders, options_updateGameConfig)

	cEnableWShaders = guiCreateCheckBox(10, 260, 180, 17, "Su Özelliği", false, false, wGraphicsMenu)
	addEventHandler("onClientGUIClick", cEnableWShaders, options_updateGameConfig)

	cEnableVShaders = guiCreateCheckBox(10, 280, 180, 17, "Araç Kalitesi", false, false, wGraphicsMenu)
...
function overlayDescSettings(button, state)
	if source == bOverlayDescription then
		if wOverlayDescSettings then
			...
			wOverlayDescSettings = guiCreateWindow(left, top, windowWidth, windowHeight, "Açıklama Seçeneklerİ", false)

	...
	local reachedBoundries=checkBoundries(thePlayer)
	    if isHoldingBall then
	        if reachedBoundries then
				outputChatBox("Don't throw the ball outside of the court.", thePlayer, 255,0,0)
	        else
				local px, py, pz = getElementPosition(thePlayer)
...
function resourceStart(source)
   ...
   makeBasket(2130.5087890625, -1820.03515625) -- Idlewood ballas
   makeBasket( 2114.2919921875, -1820.3203125) -- Idlewood ballas
   ...
   makeBoundry(-1089.9609375, -743.4794921875, 25, 13,0,0) -- San Andreas Detention Center west court
   makeBoundry(-1032.8837890625, -742.7236328125, 25, 13,0,0) -- San Andreas Detention Center east court
   makeBoundry(2113.4914550781, -1825.8786621094, 18, 11.5,0,0) -- Idlewood ballas
...
function newBallPls()
    ...
    newBall(2122.4836425781, -1820.0690917969, 12.17343788147) --Idlewood ballas
end
...
function basketHelp(thePlayer, commandName)
	for _,boundryShape in pairs(boundries) do
        if isElementWithinColShape(thePlayer,boundryShape) then
			...
			outputChatBox("/shootball: To shoot the ball towards the closest basket.", thePlayer)
			outputChatBox("/passball: To pass the ball in the direction you're facing.", thePlayer)
			outputChatBox("To dunk: Just run to the basket whilst holding the ball.", thePlayer)
		end
	end
end

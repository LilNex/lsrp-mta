...
function showVehicleMenu()
	...
	if(getElementModel(vehicle) == 508) then 
		...
		if inCar then 
			row.cook = rightclick:addRow("Cook")
		addEventHandler("onClientGUIClick", row.cook, function(button,state)
				if not getVehicleEngineState(vehicle) then
				-- if exports.global:hasItem(localPlayer,570) then 
					triggerServerEvent("bbad:cook",localPlayer,vehicle)
				else 
					outputChatBox("Turn off the engine first.",100,0,0)
				end
				-- else 
					-- outputChatBox("You don't have a tube furnace to install it in the camping car",220,0,0)
				-- end 
		end, false)

		end
		if getElementData(localPlayer, "faction") == 4 or getElementData(localPlayer, "job") == 5 or getElementData(localPlayer,"dbid") == getElementData(vehicle,"owner") then 
		row.tool1 = rightclick:addRow("Install Tube Furnance")
		addEventHandler("onClientGUIClick", row.tool1, function(button,state)
				if not getElementData(localPlayer, "faction") == 4 or not getElementData(localPlayer, "job") == 5 then 
					
					outputChatBox("You need a mechanic to install that on your car",230,0,0)
					return
				end
				if exports.global:hasItem(localPlayer,570) then 
					triggerServerEvent("bbad:installTool1",localPlayer,vehicle)
				else 
					outputChatBox("You don't have a tube furnace to install it in the camping car",220,0,0)
				end 
		end, false)

		row.tool2 = rightclick:addRow("Install Chemistry Tubes")
		addEventHandler("onClientGUIClick", row.tool2, function(button,state)
				if exports.global:hasItem(localPlayer,571) then 
					triggerServerEvent("bbad:installTool2",localPlayer,vehicle)
				else 
					outputChatBox("You don't have chemistry tubes to install it in the camping car",220,0,0)
				end 
		end, false)

		row.tool3 = rightclick:addRow("Install Catalyser")
		addEventHandler("onClientGUIClick", row.tool3, function(button,state)
				if exports.global:hasItem(localPlayer,572) then 
					triggerServerEvent("bbad:installTool3",localPlayer,vehicle)
				else 
					outputChatBox("You don't have a catalyser to install it in the camping car",220,0,0)
				end 
		end, false)
		end
	end 

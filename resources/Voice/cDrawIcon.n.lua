...
function dId (x )
	dbid = getElementData(x,"dbid")
	
end

  addEventHandler ( "onClientPlayerSpawn", getLocalPlayer(), dId )

addEventHandler("onRessourceStart",root,function()
	dId(source)
end)
...
function dxDrawVoiceLabel ( player, index, color )
	...
	-- shadows
	dId(player)
	dxDrawText (dbid, px + 1, py + 1, px, py, ICON_TEXT_SHADOW, scale )

	-- text

	dxDrawText ( dbid, px, py, px, py, color, scale )

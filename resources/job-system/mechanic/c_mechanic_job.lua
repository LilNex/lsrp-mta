...
function displayMechanicJob()
	...
	outputChatBox("#FF9933Utilisez le menu #FF000000 clic droit#FF9933 pour voir les services que vous pouvez fournir.", 255, 194, 15, true)
...
function mechanicWindow(vehicle)
	...
	local job = tonumber(getElementData(getLocalPlayer(), "job"))
	...
	if factionType == 7 or job == 5 then -- Mechanic Faction / Adams]]

		if not vehicle then

			outputChatBox("Vous devez sélectionner un véhicule.", 255, 0, 0)
			...
			if not (wMechanic) then
				...
				wMechanic = guiCreateWindow(X, Y, Width, Height, "Options Mécaniques", false )
				...
				bMechanicTwo = guiCreateButton( 0.05, y, 0.9, 0.1, "Réparer - RepairKit Needed", true, wMechanic )
				...
				if (getElementData(source,"faction")==4) then
					...
					bMechanicThree = guiCreateButton( 0.05, y, 0.9, 0.1, "Changer les pneus - " .. mechcost .. "$", true, wMechanic )
				...
				--if (playerDimension > 0) then
					...
					if faction == (4 or 74) or ( tonumber( getElementData(vehicle, "job") or 0 )) == 0   then

						mechcost = 1000



					if (getElementData(source,"faction")==75) or job == 5 then
                    ...
                    mechcost =  2000
						...
						bMechanicFour = guiCreateButton( 0.05, y, 0.9, 0.1, "Repeindre le véhicule - " .. mechcost .. "$", true, wMechanic )
				...
				if (getElementData(source,"faction")==73) then
                    ...
                    mechcost =  2350
						...
						bMechanicSix = guiCreateButton( 0.05, y, 0.9, 0.1, "Paintjob - " .. mechcost .."$", true, wMechanic )
				...
				if (getElementData(source,"faction")==73) then
                    ...
                    mechcost =  1450
						...
						bMechanicNine = guiCreateButton( 0.05, y, 0.9, 0.1, "Remplacer les lumières - ".. mechcost .."$", true, wMechanic )
					...
					if getVehicleUpgradeOnSlot(vehicle, 8) ~= 0 and faction == 4 then
						...
						bMechanicSeven = guiCreateButton( 0.05, y, 0.9, 0.1, "Enlever NOS", true, wMechanic )
					...
					if factionType == 7  or job == 5 then
					...
					if (getElementData(source,"faction")==4) then
						...
						if not getElementData(vehicle, "tinted") then
							...
							bMechanicEight = guiCreateButton( 0.05, y, 0.9, 0.1, "Mettre des vitres teintées - " .. mechcost .."$", true, wMechanic )
							...
							bMechanicEight = guiCreateButton( 0.05, y, 0.9, 0.1, "Enlever les vitres teintées - 500$", true, wMechanic )
						...
						if not getElementData(vehicle, "gps", 1) then

							bMechanicEleven = guiCreateButton( 0.05, y, 0.9, 0.1, "Mettre un GPS - " .. mechcost .."$", true, wMechanic )

							addEventHandler( "onClientGUIClick", bMechanicEleven, ajouterGPS, false)

							y = y + 0.1

						else

							bMechanicEleven = guiCreateButton( 0.05, y, 0.9, 0.1, "Retirer le GPS - " .. mechcost .."$", true, wMechanic )

							addEventHandler( "onClientGUIClick", bMechanicEleven, enleverGPS, false)
				...
				bMechanicClose = guiCreateButton( 0.05, 0.85, 0.9, 0.1, "Fermer", true, wMechanic )
...
function ajouterGPS()
	...
	triggerServerEvent("gpsVehicle", getLocalPlayer(), currentVehicle, 1)
...
function enleverGPS()
	...
	triggerServerEvent("gpsVehicle", getLocalPlayer(), currentVehicle, 2)
...
function tyreWindow()
	...
	if not (wTyre) then
		...
		wTyre = guiCreateWindow(X+100, Y, Width, Height, "Sélectionnez un pneu à mettre.", false )
		...
		if getVehicleType(currentVehicle) ~= "Bike" then
			...
			bTyreOne = guiCreateButton( 0.05, 0.1, 0.45, 0.35, "Avant gauche", true, wTyre )
			...
			bTyreTwo = guiCreateButton( 0.05, 0.5, 0.45, 0.35, "Arrière Gauche", true, wTyre )
			...
			bTyreThree = guiCreateButton( 0.5, 0.1, 0.45, 0.35, "Avant Droit", true, wTyre )
			...
			bTyreFour = guiCreateButton( 0.5, 0.5, 0.45, 0.35, "Arrière Droite", true, wTyre )
			...
			bTyreOne = guiCreateButton( 0.05, 0.1, 0.9, 0.35, "Avant", true, wTyre )
			...
			bTyreThree = guiCreateButton( 0.05, 0.5, 0.9, 0.35, "Arrière", true, wTyre )
		...
		bTyreClose = guiCreateButton( 0.05, 0.9, 0.9, 0.1, "Fermer", true, wTyre )
...
function paintColor( colorIndex )
	...
	if exports.colorblender:isPickerOpened("paintjob") then
		...
		outputChatBox("Tu es encore en train d'éditer une autre couleur, imbécile.", 255, 0 ,0)
	...
	exports.colorblender:openPicker("paintjob", string.format("#%02X%02X%02X", r, g, b) , "Choisissez une couleur pour votre véhicule:")
...
function paintWindow()
	...
	if not (wPaint) then
		...
		wPaint = guiCreateWindow(windowX, windowY, windowWidth, windowHeight, "Peinture de Véhicule", false )

		bPaintColor1 = guiCreateButton( 0.05, 0.1, 0.45, 0.2, "Mettre couleur 1", true, wPaint )
		...
		bPaintColor2 = guiCreateButton( 0.55, 0.1, 0.45, 0.2, "Mettre couleur 2", true, wPaint )
		...
		bPaintColor3 = guiCreateButton( 0.05, 0.4, 0.45, 0.2, "Mettre couleur 3", true, wPaint )
		...
		bPaintColor4 = guiCreateButton( 0.55, 0.4, 0.45, 0.2, "Mettre couleur 4", true, wPaint )
		...
		bPaintSubmit = guiCreateButton( 0.05, 0.75, 0.90, 0.1, "Sauvegarder", true, wPaint )
		...
		bPaintClose = guiCreateButton( 0.05, 0.85, 0.90, 0.1, "Fermer", true, wPaint )
...
function replaceLightsWindow()
	...
	exports.colorblender:openPicker("headlights", string.format("#%02X%02X%02X", r, g, b) , "Choisis une couleur pour les lumières:")
...
function paintjobWindow()
	...
	if not (wPaintjob) then
		...
		wPaintjob = guiCreateWindow(X+100, Y, Width, Height, "Choisis un nouveau Paintjob.", false )
		...
		bPaintjobClose = guiCreateButton( 0.05, 0.9, 0.9, 0.1, "Fermer", true, wPaintjob )

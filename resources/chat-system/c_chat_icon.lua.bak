								...
								if hasPerk and perkValue then
									if tonumber(perkValue) then
										...
										if perkValue ~= 1 then
											...
											iconSizeW, iconSizeH = 230, 135
											local bgSize = 5
											dxDrawImage(screenX, screenY, (iconSizeW+bgSize)*0.5 / dist, (iconSizeH+bgSize)*0.5 / dist, chatIcon, 0, 0, 0, tocolor(0,0,0,255))
										end
									end
								end
							end

							local draw = dxDrawImage(screenX, screenY, iconSizeW*0.5 / dist, iconSizeH*0.5 / dist, chatIcon)
						end
					end
				end
			else
...
function updateTypingIcon()	
	...
	outputDebugString("updateTypingIcon "..state)
	if (state == "1") then
...
addEventHandler("accounts:settings:graphic_typingicon", getRootElement(), updateTypingIcon)
		...
		elseif theChannel > 1 or radioID < 0 then
			...
			if radioID == -1 then
				local teams = {
					...
					getTeamFromName("Rapid Towing"),
					getTeamFromName("Government of Los Santos"),
					getTeamFromName("Bank Of Los Santos"),
...
function chatMain(message, messageType)
	...
	if (messageType == 1 or not (isPedDead(source))) and (logged==1) and not (messageType==2) then -- Player cannot chat while dead or not logged in, unless its OOC
		...
		if (messageType==0) then
			local languageslot = getElementData(source, "languages.current")
			local language = getElementData(source, "languages.lang" .. languageslot)
			if getElementData(source, "race") == 3 then
				language = 40
			end
...
function govAnnouncement(thePlayer, commandName, ...)
	...
	if (theTeam) then
		...
		if (teamID==1 or teamID==2 or teamID==3 or teamID==9 or teamID==59 or teamID==115 or teamID==121 or teamID==20 or teamID==81 or teamID==102 or teamID==12 or teamID==4 or teamID==64) then
			local message = table.concat({...}, " ")
...
function departmentradio(thePlayer, commandName, ...)
	...
	if (theTeam)  or (tollped) then
		...
		if (teamID==1 or teamID==2 or teamID==3 or teamID==4 or teamID==5 or teamID==12 or teamID==12 or tollped) then --47=FAA 64=SAPT
			if (...) then
				local message = table.concat({...}, " ")
				radio(thePlayer, -1, message)
					...
					if (logged==1) and (targetOOCEnabled==1) then
						...
						if exports.integration:isPlayerTrialAdmin(thePlayer) then
							...
							if getElementData(thePlayer, "hiddenadmin") then
								outputChatBox("(( "..exports.global:getPlayerFullIdentity(thePlayer)..": " .. message .. " ))", arrayPlayer, 255, 150, 0)
							else
								outputChatBox("(( "..exports.global:getPlayerFullIdentity(thePlayer)..": " .. message .. " ))", arrayPlayer, 255, 150, 0)
							end
                        else
...
function scripterChat(thePlayer, commandName, ...)
    ...
    if(logged==1) and (exports.integration:isPlayerTester(thePlayer))  then
        if not (...) then
            outputChatBox("SYNTAX: /dev [Message]", thePlayer, 255, 194, 14)
        else
            local message = table.concat({...}, " ")
            local players = exports.pool:getPoolElementsByType("player")
...
addCommandHandler("dev", scripterChat, false, false)
...
function localOOC(thePlayer, commandName, ...)
	...
	if (logged==1) and not (isPedDead(thePlayer)) then
		...
		elseif (muted==1) then
            ...
            if not hiddenadmin == 1 then
				result, affectedElements = exports.global:sendLocalText(thePlayer, "[OOC Chat] " ..getPlayerName(thePlayer).. ": ( " .. message .. " )", r,b,g)
...
local togState = { }
function toggle(thePlayer, commandName, State)
				...
				for index, arrayPlayer in ipairs( getElementsByType( "player" ) ) do
					if isElement( arrayPlayer ) then
						...
						elseif getPlayerTeam( arrayPlayer ) == theTeam and getElementData(arrayPlayer, "loggedin") == 1 and getElementData(arrayPlayer, "chat-system:blockF") ~= 1 then
							table.insert(affectedElements, arrayPlayer)
							outputChatBox("((Faction)) " .. playerName .. ": " .. message, arrayPlayer, 110, 255, 231)
						end
					end
				end
...
function adminChat(thePlayer, commandName, ...)
	...
	if(logged==1) and (exports.integration:isPlayerTrialAdmin(thePlayer)  or exports.integration:isPlayerSupporter(thePlayer))  then
		if not (...) then
			outputChatBox("SYNTAX: /is [Message]", thePlayer, 255, 194, 14)
		else
			local affectedElements = { }
			local message = table.concat({...}, " ")
			local players = exports.pool:getPoolElementsByType("player")
			...
			for k, arrayPlayer in ipairs(players) do
				...
				if logged==1 and (exports.integration:isPlayerTrialAdmin(arrayPlayer) or exports.integration:isPlayerSupporter(arrayPlayer)) then
					table.insert(affectedElements, arrayPlayer)
					outputChatBox("[Interne Staff] ".. adminTitle .." " .. username .. " : " .. message, arrayPlayer, 0, 165, 247)
				end
			end
...
addCommandHandler("is", adminChat, false, false)
...
addCommandHandler("ua", leadAdminChat, false, false)
...
function fmtChat(thePlayer, commandName, ...)
	...
	if(logged==1) and exports.integration:isPlayerFMT(thePlayer) then
		if not (...) then
			outputChatBox("SYNTAX: /".. commandName .. " [Message]", thePlayer, 255, 194, 14)
		else
			local affectedElements = { }

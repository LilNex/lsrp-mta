		...
		if (vehicle) then
			...
			if (getVehicleType(vehicle) == "Boat" or getVehicleType(vehicle) == "Plane" or getVehicleType(vehicle) == "Helicopter") then
				...
				if(getVehicleType(vehicle) == "Plane" or getVehicleType(vehicle) == "Helicopter") then
					...
					local airinfo = "ALT: "..tostring(altitude).."    HDG: "..tostring(heading)
...
function drawFuel()
	if getElementData(localPlayer,"speedo") ~= "0" and not isPlayerMapVisible() and exports.hud:isActive() then
		...
		if (vehicle) then
			...
			local FuelPer = (fuel/exports["fuel-system"]:getMaxFuel(vehicle))*100
			...
			dxDrawText(math.floor(FuelPer).."%", x - 215, y - 110.5, 5, 5, tocolor (255,255,255, 200), 1 )

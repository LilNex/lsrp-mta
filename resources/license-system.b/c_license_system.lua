...
function showLicenseWindow()
	...
	bBuyLicense = guiCreateButton(0.05+0.3, 0.85, 0.3, 0.1, "Buy License & Skip Test (1GC)", true, wLicense)
		...
		if (source==bAcceptLicense) or (source==bBuyLicense)then
			...
			if (source==bAcceptLicense) then
				if not exports.global:hasMoney( getLocalPlayer(), licensecost ) then
					exports.hud:sendBottomNotification(localPlayer, "Department of Motor Vehicles", "You cannot afford $"..licensecost.." this license." )
					return false
				end
			end

...
function main ( )
	...
	local impounds = {}
	local lanes = exports['tow-system']:getImpoundLanes()
	for i, lane in pairs(lanes) do
		...
		if tonumber(lane.veh) ~= 0 then
			...
			if veh and getElementData(veh, "impounder") == oneLane.impounder and getElementData(veh, "Impounded") ~= 0 then
				...
				exports.mysql:query_free("UPDATE leo_impound_lot SET veh=0, release_date=NULL, fine=0 WHERE veh="..lane.veh)
			end
		end
		table.insert(impounds, oneLane)
	end
	
	triggerClientEvent( source, resourceName .. ":main", getRootElement(), warrants, apb, impounds, calls ) 
end
	...
	elseif queryType == 0 then --Person
		...
		if exports.mysql:num_rows( result ) > 0 then
			...
			if tonumber( criminal.wanted ) == 1 then
				local result3 = exports.mysql:fetch_assoc( "SELECT `user` FROM mdc_users WHERE `id` = '".. exports.mysql:escape_string( criminal.wanted_by ) .."' LIMIT 1" ) --We need to figure out the wanted by's name!
				criminal.wanted_by = "No-one"
				if result3 and result3.user ~= mysql_null() then
			...
			if(canSeePilotStuff[getElementData(source, "mdc_org")]) then

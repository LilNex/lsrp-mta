...
function openRadioManager(defaultStations1, donorStations1)
	if true then--canAccessManager() then
		...
		if gui.main and isElement(gui.main) then
			...
			gui.main = guiCreateWindow(x,y,w,h,"Radio Station Manager v2.1 | Loading..",false)
			...
			gui.buyNew = guiCreateButton(0.0135,0.9135,0.32476,0.0675,"Purchase a new station (10 GC)",true,gui.main)
		...
		function( button )
			if button == "left" then
				...
				if row ~= -1 and col ~= -1 then
					...
					if owner == username or exports.integration:isPlayerSeniorAdmin(localPlayer) then
		...
		function( button )
			if button == "left" then
				...
				if row ~= -1 and col ~= -1 then
					local username = getElementData(localPlayer, "account:username")
					local owner = guiGridListGetItemText( grid.donorStations , row, 5 )
					if owner == username then --or exports.integration:isPlayerSeniorAdmin(localPlayer) then
						local id = guiGridListGetItemText( grid.donorStations , row, 1 )
						local name = guiGridListGetItemText( grid.donorStations , row, 2 )
						local ip = guiGridListGetItemText( grid.donorStations , row, 3 )
						local owner = guiGridListGetItemText( grid.donorStations , row, 5 )
...
function openNewStation(id, name, ip, state, order, buyNew)
	...
	gui.bMoveUp = guiCreateButton(margin, margin+lineH/2+lineH*3, buttonW , lineH, buyNew and "Move up (1GC)" or "Move up",false,gui.wNewStation)
	...
	gui.bMoveDown = guiCreateButton(margin+buttonW, margin+lineH/2+lineH*3, buttonW , lineH, buyNew and "Move down (1GC)" or "Move down",false,gui.wNewStation)
...
function renewStation(station)
	...
	local bH = 30
	local buttonW = (w-margin*2)/buttons
	gui.b7days = guiCreateButton(margin, margin+lineH/2+lineH*6, buttonW , bH, "7 days(3GC)",false,gui.wRenewStation)
	...
	gui.b30days = guiCreateButton(margin+buttonW, margin+lineH/2+lineH*6, buttonW , bH, "30 days(10GC)",false,gui.wRenewStation)
	...
	gui.b90days = guiCreateButton(margin+buttonW*2, margin+lineH/2+lineH*6, buttonW , bH, "90 days(25GC)",false,gui.wRenewStation)

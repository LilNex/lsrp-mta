...
function takeRemainingCrates(vehicle)
	if vehicle then
		...
		exports.anticheat:changeProtectedElementDataEx(vehicle, "job-system-trucker:loadedSupplies",0, true )
		if weight > 0 then
			playSoundFX(vehicle)
			return weight
		else
...
function checkTruckingEnterVehicle(thePlayer, seat)
	...
	if seat == 0 and getElementData(source,"job") == 1 and getElementData(thePlayer,"job") == 1 then
		if true then --exports.integration:isPlayerScripter(thePlayer) then
...
function startLoadingUp()
	...
	if truck then
		...
		if crateWeight > 0 and exports["item-system"]:giveItem( vehicle, 121, crateWeight) then
			playSoundFX(vehicle)
...
function getCurrentCrates(vehicle)
	...
	exports.anticheat:changeProtectedElementDataEx(vehicle, "job-system-trucker:loadedSupplies",count,true)
	return count
end
...
function checkIfPlayerTruckHasEnoughtShit()
	...
	if success then
		...
		exports.hud:sendBottomNotification(source, "RS Haul Operator: ", "Customer paid you $"..formartedEarned.." for "..droppedWeight.." Kg of supplies you unloaded from the truck.")
		
		playSoundFX(vehicle)
		
	else
		exports.hud:sendBottomNotification(source, "RS Haul Operator: ", droppedWeight.." Return to RS Haul's warehouse first.")
	end
	...
	if getVehicleOccupant(veh) == thePlayer then
		...
		if currentSpot then
			...
			if currentSpot[4] then --Weight Required
				...
				exports.anticheat:changeProtectedElementDataEx(thePlayer, "job-system-trucker:currentRoute", currentSpot)
			end

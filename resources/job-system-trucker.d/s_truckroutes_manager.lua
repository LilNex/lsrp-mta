...
function addactualorder(thePlayer, commandName, orderWeight, InteriorID, ...)
	if (exports.integration:isPlayerScripter(thePlayer)) then
		...
		if mysql:query_free("INSERT INTO `jobs_trucker_orders` SET `orderX`='"..tostring(x).."', `orderY`='"..tostring(y).."', `orderZ`='"..tostring(z).."', `orderWeight`='"..tostring(math.floor(tonumber(orderWeight))).."', `orderName`='"..tostring(orderName):gsub("'","''").."', `orderInterior`='"..tostring(math.floor(tonumber(InteriorID))).."' ") then

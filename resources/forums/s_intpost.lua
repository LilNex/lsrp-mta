...
function intPost(intNotes, targetPlayer, targetPlayerName, interiorID)
	...
	--[[local threadTitle = ""
	local postID = exports.mysql:forum_query_insert_free("insert into `post` set `threadid`='6204', `parentid`='41243', `username`='Chuevo', `userid`='2', `title`='" .. threadTitle .. "', `dateline`=unix_timestamp(), `pagetext`='[B]Players Name: [/B]" ..getPlayerName(targetPlayer):gsub("_"," ").." (" ..targetUsername..") <br>[B]Date: [/B] ".. date .. " <br>[B]Time: [/B] " ..time.. " <br> [B]House ID: [/B] " .. interiorID .. "<br>[B]Approved by: [/B]" .. adminUsername .. " <br>[B]Contents taken: [/B]<br> " .. notes .. "', `allowsmilie`='1', `showsignature`='0', `ipaddress`='127.0.0.1', `iconid`='0', `visible`='1', `attach`='0', `infraction`='0', `reportthreadid`='0'")
	local secondShit = exports.mysql:forum_query_insert_free("update `thread` set `lastpostid`='" .. postID .. "', `lastpost`=unix_timestamp(), `replycount`='replycount + 1', `postercount`='postercount + 1', `lastposter`='Chuevo', `lastposterid`='2', `dateline`='unix_timestamp()' WHERE `threadid`= '6204'")
	exports.mysql:forum_query_free("update `user` set posts = posts + 1 where userid = '2'")
	exports.mysql:forum_query_free("UPDATE forum set replycount = replycount + 1, lastpost = unix_timestamp(), lastposter = 'Chuevo', lastposterid='2', lastpostid='"..postID.."', lastthread='Interior Break-ins' ,lastthreadid='6204', replycount = replycount + 1 WHERE forumid = 40")
	exports.mysql:forum_query_free("UPDATE forum set replycount = replycount + 1, lastpost = unix_timestamp(), lastposter = 'Chuevo', lastposterid='2', lastpostid='"..postID.."', lastthread='Interior Break-ins' ,lastthreadid='6204', replycount = replycount + 1 WHERE forumid = 31")
	exports.mysql:forum_query_free("UPDATE forum set replycount = replycount + 1, lastpost = unix_timestamp(), lastposter = 'Chuevo', lastposterid='2', lastpostid='"..postID.."', lastthread='Interior Break-ins' ,lastthreadid='6204', replycount = replycount + 1 WHERE forumid = 38")
	]]

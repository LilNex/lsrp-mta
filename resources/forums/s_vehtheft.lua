...
function theftPost(noteEdit, EngineCheck, KeyCheck, targetPlayerName, targetVehicle, targetPlayer, SellVehCheck, ChangeLockCheck)
	...
	if EngineCheck then
		...
		exports['vehicle-system']:reloadVehicle(tonumber(targetVehicle))
	...
	if ChangeLockCheck then
		exports.logs:logMessage( "[VEHICLE] " .. getPlayerName( source ) .. " changed the lock for Vehicle #" .. targetVehicle .. ".", 16) 
	...
	exports["vehicle-manager"]:addVehicleLogs(targetVehicle, "approved theft", source)
	local vehicleName = getVehicleNameFromModel(model)
	--[[local threadTitle = exports.mysql:escape_string("")
	local postID = exports.mysql:forum_query_insert_free("insert into `post` set `threadid`='6205', `parentid`='41245', `username`='Chuevo', `userid`='2', `title`='" .. threadTitle .. "', `dateline`=unix_timestamp(), `pagetext`='[B]Players Name:[/B] " .. getPlayerName(targetPlayer):gsub("_", " ").. " (" ..targetUsername.. ") <br>[B]Date: [/B]" ..date.. "<br>[B]Vehicle Owner: [/B]" ..actualOwner:gsub("_", " ").. "<br>[B]Vehicle Model:[/B] " .. vehicleName .. " <br>[B]Vehicle ID:[/B] "..targetVehicle.."<br>[B]Approved by:[/B] " .. getPlayerName(source):gsub("_", " ").. " (" ..adminUsername..")<br>[B]Notes:[/B]<br> " ..notes.. "<br><br>[B]Engine started: [/B]" ..enginestarted.. "<br>[B]Keys given:[/B] " ..keygiven.. "<br>[B]Locks changed: [/B]" ..lockchanged.. "<br>[B]Vehicle sold: [/B]" ..vehsold.. "', `allowsmilie`='1', `showsignature`='0', `ipaddress`='127.0.0.1', `iconid`='0', `visible`='1', `attach`='0', `infraction`='0', `reportthreadid`='0'")
	local secondShit = exports.mysql:forum_query_insert_free("update `thread` set `lastpostid`='" .. postID .. "', `lastpost`=unix_timestamp(), `replycount`='replycount + 1', `postercount`='postercount + 1', `lastposter`='Chuevo', `lastposterid`='2', `dateline`='unix_timestamp()' WHERE `threadid`= '6205'")
	exports.mysql:forum_query_free("update `user` set posts = posts + 1 where userid = '2'")
	exports.mysql:forum_query_free("UPDATE forum set replycount = replycount + 1, lastpost = unix_timestamp(), lastposter = 'Chuevo', lastposterid='2', lastpostid='"..postID.."', lastthread='Vehicle Thefts' ,lastthreadid='6205', replycount = replycount + 1 WHERE forumid = 41")
	exports.mysql:forum_query_free("UPDATE forum set replycount = replycount + 1, lastpost = unix_timestamp(), lastposter = 'Chuevo', lastposterid='2', lastpostid='"..postID.."', lastthread='Vehicle Thefts' ,lastthreadid='6205', replycount = replycount + 1 WHERE forumid = 31")
	exports.mysql:forum_query_free("UPDATE forum set replycount = replycount + 1, lastpost = unix_timestamp(), lastposter = 'Chuevo', lastposterid='2', lastpostid='"..postID.."', lastthread='Vehicle Thefts' ,lastthreadid='6205', replycount = replycount + 1 WHERE forumid = 38")
	]]

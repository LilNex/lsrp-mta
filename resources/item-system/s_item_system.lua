...
function useItem(itemSlot, additional)
	...
	if itemID then
		...
		if (itemID==1) then -- hotdog
				      ...
				      if ( ( (getElementData(source,"Hunger") or 100)+30) <= 100) then

      		setElementData(source,"Hunger", (getElementData(source,"Hunger") or 100)+0 )
			...
			exports.global:giveItem ( source, 216, 1 )
		...
		elseif (itemID==8) then -- sandwich
		      ...
		      if ( ( (getElementData(source,"Hunger") or 100)+35) <= 100) then

      		setElementData(source,"Hunger", (getElementData(source,"Hunger") or 100)+35 )
		...
		elseif (itemID==9) then -- sprunk
				      ...
				      if ( ( (getElementData(source,"Thirst") or 100)+35) <= 100) then

      		setElementData(source,"Thirst", (getElementData(source,"Thirst") or 100)+35 )
		...
		elseif (itemID==17) then -- watch
			...
			triggerEvent('sendAme', source, "looks at their watch.")
		...
		elseif (itemID==277) then -- Cannabis
		  ...
		  triggerEvent('sendAme', source, "sniffs some Cannabis.")
		...
		elseif (itemID==555) then

			giveHealth(source, 20)

			takeItemFromSlot(source, itemSlot)

			triggerEvent('sendAme', source, "smokes some methamphetamine.")
...
elseif (itemID==276) then

			local dbid = getElementDimension( source )

            if hasItem( source, 278 ) and hasItem( source, 15 ) then           

			if dbid == 0 then

			exports.scr_notifs:showBoxS(source, "You are not inside an interior!", "info",5000)

            else

	if ( dbid < 19000 and ( hasItem(source, 5, dbid) or hasItem(source, 4, dbid) ) ) or ( dbid >= 20000 and hasItem(source, 3, dbid-20000) ) or ((getElementData(source,"admin_level") >= 2) and (getElementData(source,"duty_admin") == 1 )) then

local interior = getElementInterior(source)			

local x, y, z, x1, y1, z1 = getElementPosition(source)

exports['item-system']:takeItem(source, 15)

exports['item-system']:takeItem(source, 276)

exports['item-system']:takeItem(source, 278)

			exports.global:applyAnimation(source, "bomber", "bom_plant", 4000, false, true, true)

			triggerEvent('sendAme', source, "Begins to plant cannabis as he starts adding water then.")

setTimer ( function()			

local weed = createObject ( 2901, tonumber(x)+0.04, tonumber(y), tonumber(z)-1, tonumber(x1), tonumber(y1), tonumber(z1) )

local weedscale = setObjectScale(weed, 0.4)

local potWeed = createObject ( 2203, x+0.1, y, z-0.8, x1, y1, z1 )

local scale = getObjectScale(weed)

setElementInterior(weed, interior)

setElementDimension(weed, dbid)

setElementInterior(potWeed, interior)

setElementDimension(potWeed, dbid)

setElementFrozen (weed,true)

setElementFrozen (potWeed,true)

setElementCollisionsEnabled(potWeed,true)

setElementCollisionsEnabled(weed,false)

mysql:query_free("INSERT INTO `weedpot` (`x`, `y`, `z`, `mx`, `my`, `mz`, `dimension`, `interior`, `scale`) VALUES ('"..tonumber(x).."', '"..tonumber(y).."', '"..tonumber(z).."', '"..tonumber(x).."', '"..tonumber(y).."', '"..tonumber(z).."', '"..tonumber(dbid).."', '"..tonumber(interior).."', '"..tonumber(scale).."')")
...
outputChatBox("You planted some cannabis with id: " .. result.id .. ".", source, 255, 51, 102)

setElementData(potWeed, "id", result.id)

setElementData(potWeed, "time", result.time)

setElementData(potWeed, "waterlevel", result.waterlevel)

setElementData(potWeed, "growlevel", result.growlevel)

end, db, 'SELECT id, growlevel, waterlevel, time FROM weedpot ORDER BY id DESC LIMIT 1')

end, 4000, 1 )

	else

exports.scr_notifs:showBoxS(source, "You don't have keys for this interior!", "info",5000)
			...
			exports.scr_notifs:showBoxS(source, "You need a pot and some water to plant cannabis.", "info",5000)
		...
		elseif (itemID==60) then
			...
			if (retval == 0) then
				...
				triggerEvent('sendAme', source, "places a safe.")
		...
		elseif (itemID==93) then
			...
			triggerEvent('sendAme', source, "take an aspro")
		...
		elseif (itemID==102) then --[ADDED: 2/23/10 by herbjr] Aspro (Asked for by Misha)
		...
		elseif (itemID==10016) then --[ADDED: 2/23/10 by herbjr] Aspirin (Asked for by Misha)
			...
			triggerEvent('sendAme', source, "take an aspirin")
      ...
      if getElementData(source, "Hunger") + 20 > 100 then
		...
		elseif masks[itemID] then
			...
			if mask then
				...
				if itemID == 56 then 
					-- triggerEvent("removeMask",source,source)
					exports.mask:removeMask(source)
				...
				elseif itemID == 220 then -- Mask Monkey

					triggerEvent("artifacts:remove", source, source, "maskmonkey")

				elseif itemID == 227 then -- glss

					triggerEvent("artifacts:remove", source, source, "glss")

				elseif itemID == 221 then -- Mask Owl

					triggerEvent("artifacts:remove", source, source, "maskOwl")

				elseif itemID == 222 then -- Mask Pig

					triggerEvent("artifacts:remove", source, source, "maskpig")

				elseif itemID == 224 then -- capNY

					triggerEvent("artifacts:remove", source, source, "capNY")

				elseif itemID == 249 then -- hatmacersun

					triggerEvent("artifacts:remove", source, source, "hatmacersun")

				elseif itemID == 234 then -- capbulls

					triggerEvent("artifacts:remove", source, source, "capbulls")

				elseif itemID == 235 then -- capcavalier

					triggerEvent("artifacts:remove", source, source, "capcavalier")

				elseif itemID == 237 then -- capdodgers

					triggerEvent("artifacts:remove", source, source, "capdodgers")

				elseif itemID == 238 then -- capdodgersALL

					triggerEvent("artifacts:remove", source, source, "capdodgersALL")

				elseif itemID == 236 then -- capcelctics

					triggerEvent("artifacts:remove", source, source, "capcelctics")

				elseif itemID == 226 then -- capiiTzK

					triggerEvent("artifacts:remove", source, source, "capiiTzk")

				elseif itemID == 246 then -- capCN

					triggerEvent("artifacts:remove", source, source, "capCN")

				elseif itemID == 247 then -- capdodgerscamo

					triggerEvent("artifacts:remove", source, source, "capdodgerscamo")

				elseif itemID == 241 then -- capkeine

					triggerEvent("artifacts:remove", source, source, "capkeine")

				elseif itemID == 239 then -- caplakers

					triggerEvent("artifacts:remove", source, source, "caplakers")

				elseif itemID == 240 then -- caplakersCAMO

					triggerEvent("artifacts:remove", source, source, "caplakersCAMO")

				elseif itemID == 242 then -- caplakerschwart

					triggerEvent("artifacts:remove", source, source, "caplakerschwart")

				elseif itemID == 243 then -- capphillies

					triggerEvent("artifacts:remove", source, source, "capphillies")

				elseif itemID == 244 then -- capsoxblack

					triggerEvent("artifacts:remove", source, source, "capsoxblack")

				elseif itemID == 245 then -- capsox

					triggerEvent("artifacts:remove", source, source, "capsox")

				elseif itemID == 248 then -- CapiiTz

					triggerEvent("artifacts:remove", source, source, "CapiiTz")

				elseif itemID == 223 then -- Mask zorro

					triggerEvent("artifacts:remove", source, source, "zorro")

				elseif itemID == 56 then -- ski mask

					triggerEvent("artifacts:remove", source, source, "skimask")

				elseif itemID == 216 then -- capback

					triggerEvent("artifacts:remove", source, source, "capback")

				elseif itemID == 217 then -- cowboy

					triggerEvent("artifacts:remove", source, source, "cowboy")

				elseif itemID == 218 then -- santahat

					triggerEvent("artifacts:remove", source, source, "santahat")

				elseif itemID == 219 then -- captruck

					triggerEvent("artifacts:remove", source, source, "captruck")
				...
				elseif itemID == 250 then -- band

					triggerEvent("artifacts:remove", source, source, "band")

				elseif itemID == 251 then -- band2

					triggerEvent("artifacts:remove", source, source, "band2")

				elseif itemID == 252 then -- band3

					triggerEvent("artifacts:remove", source, source, "band3")

				elseif itemID == 253 then -- band4

					triggerEvent("artifacts:remove", source, source, "band4")

				elseif itemID == 254 then -- hat

					triggerEvent("artifacts:remove", source, source, "hat")

				elseif itemID == 255 then -- cowboyred

					triggerEvent("artifacts:remove", source, source, "cowboyred")

				elseif itemID == 256 then -- cowboyblue

					triggerEvent("artifacts:remove", source, source, "cowboyblue")

				elseif itemID == 257 then -- shoping

					triggerEvent("artifacts:remove", source, source, "shoping")
			...
			elseif getPedOccupiedVehicle(source) and getElementData(getPedOccupiedVehicle(source), "job") > 0 and getVehicleOccupant(getPedOccupiedVehicle(source)) == source then
				...
				if itemID == 56 then 
					triggerClientEvent(source,"pAcao10",root)
				...
				elseif itemID == 220 then -- Mask Monkey
					...
					triggerEvent("artifacts:add", source, source, "maskmonkey")

				elseif itemID == 227 then -- glss

					triggerEvent("artifacts:add", source, source, "glss")

				elseif itemID == 221 then -- Mask Owl

					triggerEvent("artifacts:add", source, source, "maskOwl")

				elseif itemID == 222 then -- Mask Pig

					triggerEvent("artifacts:add", source, source, "maskpig")

				elseif itemID == 224 then -- capNY

					triggerEvent("artifacts:add", source, source, "capNY")

				elseif itemID == 249 then -- hatmacersun

					triggerEvent("artifacts:add", source, source, "hatmacersun")

				elseif itemID == 234 then -- capbulls

					triggerEvent("artifacts:add", source, source, "capbulls")

				elseif itemID == 235 then -- capcavalier

					triggerEvent("artifacts:add", source, source, "capcavalier")

				elseif itemID == 237 then -- capdodgers

					triggerEvent("artifacts:add", source, source, "capdodgers")

				elseif itemID == 238 then -- capdodgersALL

					triggerEvent("artifacts:add", source, source, "capdodgersALL")

				elseif itemID == 236 then -- capcelctics

					triggerEvent("artifacts:add", source, source, "capcelctics")

				elseif itemID == 226 then -- capiiTzk

					triggerEvent("artifacts:add", source, source, "capiiTzk")

				elseif itemID == 246 then -- capCN

					triggerEvent("artifacts:add", source, source, "capCN")

				elseif itemID == 247 then -- capdodgerscamo

					triggerEvent("artifacts:add", source, source, "capdodgerscamo")

				elseif itemID == 241 then -- capkeine

					triggerEvent("artifacts:add", source, source, "capkeine")

				elseif itemID == 239 then -- caplakers

					triggerEvent("artifacts:add", source, source, "caplakers")

				elseif itemID == 240 then -- caplakersCAMO

					triggerEvent("artifacts:add", source, source, "caplakersCAMO")

				elseif itemID == 242 then -- caplakerschwart

					triggerEvent("artifacts:add", source, source, "caplakerschwart")

				elseif itemID == 243 then -- capphillies

					triggerEvent("artifacts:add", source, source, "capphillies")

				elseif itemID == 244 then -- capsoxblack

					triggerEvent("artifacts:add", source, source, "capsoxblack")

				elseif itemID == 245 then -- capsox

					triggerEvent("artifacts:add", source, source, "capsox")

				elseif itemID == 248 then -- CapiiTz

					triggerEvent("artifacts:add", source, source, "CapiiTz")

				elseif itemID == 223 then -- Mask zorro

					triggerEvent("artifacts:add", source, source, "zorro")

				elseif itemID == 56 then -- ski mask

					triggerEvent("artifacts:add", source, source, "skimask")

				elseif itemID == 216 then -- capback

					triggerEvent("artifacts:add", source, source, "capback")

				elseif itemID == 217 then -- cowboy

					triggerEvent("artifacts:add", source, source, "cowboy")

				elseif itemID == 218 then -- santahat

					triggerEvent("artifacts:add", source, source, "santahat")

				elseif itemID == 219 then -- captruck

					triggerEvent("artifacts:add", source, source, "captruck")

				elseif itemID == 250 then -- band

					triggerEvent("artifacts:add", source, source, "band")

				elseif itemID == 251 then -- band2

					triggerEvent("artifacts:add", source, source, "band2")

				elseif itemID == 252 then -- band3

					triggerEvent("artifacts:add", source, source, "band3")

				elseif itemID == 253 then -- band4

					triggerEvent("artifacts:add", source, source, "band4")

				elseif itemID == 254 then -- hat

					triggerEvent("artifacts:add", source, source, "hat")

				elseif itemID == 255 then -- cowboyred

					triggerEvent("artifacts:add", source, source, "cowboyred")

				elseif itemID == 256 then -- cowboyblue

					triggerEvent("artifacts:add", source, source, "cowboyblue")

				elseif itemID == 257 then -- shoping

					triggerEvent("artifacts:add", source, source, "shoping")
		...
		elseif (itemID==400) then -- Bandage
			...
			triggerEvent('sendAme', source, "uses the bandage.")

			
		elseif (itemID==405) then -- Repair

			local theVehicle = getPedOccupiedVehicle (source) 
			local engine = getElementData(theVehicle, "engine")
			if engine == 0 then
			      takeItemFromSlot(source, itemSlot)
			      outputChatBox("[RepairKit] You are fixing the vehicule.", source, 0, 255, 0)
			       ...
			       triggerEvent('sendAme', source, "Repairs the vehicule using the repair kit")
			else
				outputChatBox("[RepairKit] Eroor", source, 0, 255, 0)
			end 
		...
		elseif (itemID==270) then
			...
			if exports.global:hasItem(source, 2, found) then

	   			exports.scr_notifs:showBoxS(source, "You've used the Sim, enjoy your new number now.", "info",5000)

			   triggerEvent('sendAme', source, "Turns off the phone then inserts the new Sim.")

						exports['item-system']:giveItem(source, 2, itemValue)

			   takeItemFromSlot(source, itemSlot)

						exports['item-system']:takeItem(source, 2)

		   else

   exports.scr_notifs:showBoxS(source, "You don't have a phone to use the Sim.", "info",5000)
...
function loadWeedPots(queryHandle)

local results = dbPoll(queryHandle, 0)

   for index, weeds in pairs(results) do 

    weedobject = createObject(2901, weeds.x+0.04, weeds.y, weeds.z-1, 0, 0, 0)

    potobject = createObject(2203,weeds.x+0.1,weeds.y,weeds.z-0.8,0,0,0)

	setObjectScale(weedobject,weeds.scale) 

   setElementData(potobject, "id",weeds.id)

   setElementDimension(weedobject,weeds.dimension)

  setElementInterior(weedobject,weeds.interior)

  setElementDimension(potobject,weeds.dimension)

  setElementInterior(potobject,weeds.interior)

  setElementFrozen(potobject,true)

  setElementFrozen(weedobject,true)

setElementData(potobject, "time", weeds.time)

setElementData(potobject, "waterlevel", weeds.waterlevel)

setElementData(potobject, "growlevel", weeds.growlevel)

attachElements ( weedobject, potobject )
...
dbQuery(loadWeedPots, db, 'SELECT * FROM weedpot')
...
iprint("Stopped")
   ...
   local waterlevel = getElementData(v, "waterlevel")

   local growlevel = getElementData(v, "growlevel")

   local timeleft = getElementData(v, "time")

   mysql:query_free("UPDATE weedpot SET time='" .. timeleft .. "', growlevel='" .. growlevel .. "', waterlevel='" .. waterlevel .. "' WHERE id='" .. id .. "'")
...
setTimer ( function()
   ...
   local grow = getElementData(v, "growlevel")

   if grow <= 100 then

  setElementData(v, "growlevel", tonumber(grow)+2)
 ...
 setElementData(v, "time", tonumber(timeleft)-2)
 ...
 setElementData(v, "waterlevel", tonumber(waterlevel)-2)

 end

 if waterlevel < 0 then

 setElementData(v, "waterlevel", 0)

 end

  if grow > 100 then

 setElementData(v, "growlevel", 100)
   ...
   for i,t in pairs(getElementsByType("object")) do

   if getElementModel(t) == 2901 then

    	local weedscale = getObjectScale( t )
	...
	setObjectScale(t, weedscale + 0.01)

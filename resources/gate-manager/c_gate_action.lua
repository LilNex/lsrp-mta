...
function fopenTheGate(commandName, password)
	...
	for _, theGate in ipairs(getElementsByType("object")) do
		...
		if isGate then
			...
			if (canPlayerReachGate(theGate, localPlayer)) then
				...
				triggerServerEvent("gate:forcetrigger", theGate)
...
addEvent("gate:ctrigger",true)
addEventHandler("gate:ctrigger",root,fopenTheGate)
addCommandHandler("fgate", fopenTheGate)

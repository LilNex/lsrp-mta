...
function getPlayerScripterLevel(thePlayer)
	return (isElement( thePlayer ) and getElementData(thePlayer, "scripter_level")) or 0
end
...
function getPlayerAdminTitle(thePlayer)
	if isElement(thePlayer) then
		...
		if exports.integration:isPlayerTrialAdmin(thePlayer) then
			...
			if (getPlayerAdminLevel(thePlayer)) == 10 then
				text = "Senior Scripter"
			end
		...
		elseif exports.integration:isPlayerVehicleConsultant(thePlayer) then
			return "VCT Leader"
		elseif exports.integration:isPlayerFMT(thePlayer) then
			return "FMT"

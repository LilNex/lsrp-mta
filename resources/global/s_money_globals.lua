...
function giveDirtyMoney(player,ammount)
	-- 1 = ID   2 = Value
	...
	setElementData(player,"dirtymoney",total)
	giveItem(player,300,total)
	return true
end
function hasDirtyMoney(player, ammount)
giveDirtyMoney(player,0)
local inv = exports["item-system"]:getItems(player)
local total = 0
	for k,v in ipairs(inv) do 
		if v[1] == 300 then
	...
	if total >= tonumber(ammount) then 
		return total
	else 
		return false 
	end 
...
function takeDirtyMoney(player, ammount)
giveDirtyMoney(player,0)
local inv = exports["item-system"]:getItems(player)
local total = 0
	for k,v in ipairs(inv) do 

...
function startGUI(player)
	...
	if (logged==1) then
		if exports.factions:isPlayerInFaction(player, 1) or exports.factions:isPlayerInFaction(player, 81) and getElementData(player, "duty") > 0 or exports.integration:isPlayerTrialAdmin(player) then
					...
					for key, value in ipairs(players) do
						if getElementData(value, "dbid") == tonumber(t[row][2]) then
                            ...
                            if not getElementData(value, "adminjailed") then
                                ...
                                setElementPosition(value, 1520.853515625, -1675.185546875, 13.546875)
...
addEventHandler("addPrisoner", resourceRoot,
	function( name, cell, days, hours, charges, fine, online )
		...
		local days = tonumber(days)*60
        if cells[cell].type == 1 then
            onlineTime = ( (days + tonumber(hours)) ) * onlineRatio -- In minutes
            offlineTime = ( (days + tonumber(hours)) ) * offlineRatio -- In minutes
		    jailTime = ( r.timestamp + (offlineTime)   ) -- For timestamp usage
        else
            jailTime = ( r.timestamp + (tonumber(hours) + days) * 60  )
            onlineTime = 0
        end
		...
		if tonumber(value[2]) == tonumber(getElementData(client, "dbid")) then
    		...
    		if remainingtime<=0 and tonumber(onlineMinutes)<=0 then
                ...
                if not getElementData(client, "adminjailed") then
                    local cell = value[8]
                    local rel = releaseLocations[cells[cell].location]
                    setElementPosition(client, 1520.853515625, -1675.185546875, 13.546875)
			...
			for key, value in ipairs(t) do
				if tonumber(value[2]) == tonumber(getElementData(thePlayer, "dbid")) then
                    ...
                    if not getElementData(thePlayer, "adminjailed") then
                        ...
                        local rel = releaseLocations[cells[cell].location]
                        setElementPosition(thePlayer, 1520.853515625, -1675.185546875, 13.546875)
...
function sendPrisonMsg(string)
    ...
    for _, v in ipairs(getPlayersInTeam(exports.factions:getTeamFromFactionID(1))) do

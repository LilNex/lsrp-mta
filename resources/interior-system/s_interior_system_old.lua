...
function unownProperty(intid) --This function is meant to be used by the system or to be triggered from UCP remotely. Works similarly to the publicSellProperty however, it's simplier. / MAXIME / 2015.1.11
	...
	local int = mysql:query_fetch_assoc("SELECT * FROM interiors WHERE id="..intid.." LIMIT 1")
	...
	exports.logs:dbLog("SYSTEM", 37, { "in"..intid } , "FORCESELL "..intid)
	exports["interior-manager"]:addInteriorLogs(intid, "Forcesold by SYSTEM")
					...
					if interiorStatus[INTERIOR_OWNER] == getElementData(thePlayer, "dbid") or exports.integration:isPlayerAdmin(thePlayer) then
						if getElementData(targetPlayer, "dbid") ~= interiorStatus[INTERIOR_OWNER] then
							if exports.global:hasSpaceForItem(targetPlayer, 4, dbid) then
								local query = mysql:query_free("UPDATE interiors SET owner = '" .. getElementData(targetPlayer, "dbid") .. "', faction=0 WHERE id='" .. dbid .. "'")
								if query then							
									local keytype = 4
									if interiorType == 1 then
										keytype = 5
									...
									local addLog = mysql:query_free("INSERT INTO `interior_logs` (`intID`, `action`, `actor`) VALUES ('"..tostring(dbid).."', '"..commandName.." to "..exports.global:toSQL(targetPlayerName).."', '"..adminID.."')") or false
									if not addLog then
										outputDebugString("Failed to add interior logs.")
									end
...
function buypropertyForFaction(interior, cost, isHouse) --Maxime
	...
	if not mysql:query_free( "UPDATE interiors SET owner='-1', faction='"..factionId.."', locked=0 WHERE id='" .. intId .. "'") then
...
--[[function vehicleStartEnter(thePlayer)
	...
	if marker and found then -- If it's there, then you cancel, not just because the event wants us to.
		...
		exports['anticheat-system']:changeProtectedElementDataEx(thePlayer, "interiormarker", nil, nil, nil) -- If it's not found, then let them enter the car, and clear the element data so it's not there anymore.
	end
end
addEventHandler("onVehicleStartEnter", getRootElement(), vehicleStartEnter)
addEventHandler("onVehicleStartExit", getRootElement(), vehicleStartEnter)]]

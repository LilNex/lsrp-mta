...
function drawPmThump()
	if exports.hud:isActive() and getElementData(localPlayer, "loggedin") == 1 and not isPlayerMapVisible() then
		globalOffSetX = (getElementData(localPlayer, "hud:whereToDisplay") or 0 ) - 200
		...
		local posxBG = imgw+3+globalOffSetX
		local posxText = posxIcon+30
		...
		if not font then 
			font = dxCreateFont (":resources/cartwheel.otf" , 10 )
		end
		local text = '    '
		if unreads > 0 then
			if unreads <=10 then
				text = string.format("%02d", unreads)
			elseif unreads > 10 then
				text = '10+'
			end
		end
		dxDrawText( text.." Notifications!", posxText+thumpOffsetX+2, 12+globalOffSetY+2, bgw, bgh, tocolor ( 0, 0, 0, 255 ), 1, font)
		dxDrawText( text.." Notifications!", posxText+thumpOffsetX, 12+globalOffSetY, bgw, bgh, tocolor ( 255, 255, 255, 255 ), 1, font)

		if isCursorShowing() then
			local cursorX, cursorY, cwX, cwY, cwZ = getCursorPosition()
			cursorX, cursorY = cursorX * sx, cursorY * sy
			if isInBox( cursorX, cursorY, posxText+thumpOffsetX+2, posxText+thumpOffsetX+2+bgw, 12+globalOffSetY, 12+globalOffSetY+bgh) then

...
function showOpmTip()
	...
	GUIEditor.label[2] = guiCreateLabel(17, 197, 414, 30, "It costs 1 GC to activate this feature in F10 -> Premium Features.\nTo send offline private message use /opm", false, GUIEditor.window[2])
...
function startOpm(cmd, username, ...)
	if exports.donators:hasPlayerPerk(localPlayer, 37) or exports.integration:isPlayerStaff(localPlayer) then
		if not username or not (...) then
			...
			addEventHandler("onClientGUIClick", GUIEditor.button[2], function()
				if source == GUIEditor.button[2]  then
					...
					triggerServerEvent("opm:send", localPlayer, foundUsername, guiGetText(GUIEditor.memo[1]))
					closeOpm()
				end
			end)
		end

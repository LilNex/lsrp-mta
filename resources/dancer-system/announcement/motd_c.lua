...
function openMotdManager(motdList1)
	...
	if GUIEditor.window[1] and isElement(GUIEditor.window[1]) then
        ...
        showCursor(true)
        GUIEditor.window[1] = guiCreateWindow(412, 210, 800, 354, "MOTD - Message of the day v1.0 [Maxime - 2015.1.9]", false)
...
function createNewMotd(motdId)
    ...
    staffTitles = exports.integration:getStaffTitles()
    GUIEditor.window[2] = guiCreateWindow(611, 279, 800, 354, motdId and ("Edit MOTD ID #"..motdId) or "Create new MOTD", false)
    ...
    for i = 1, #staffTitles do
        ...
        for j = #team, 1, -1 do
            count = count + 1
            GUIEditor.audience[i][j] = guiCreateCheckBox(554, 80+aHeight*count, 226, 17, team[j], false, false, GUIEditor.window[2])
    ...
    count = count + 1
    GUIEditor.audience[0] = guiCreateCheckBox(554, 80+aHeight*count, 226, 17, "Player", false, false, GUIEditor.window[2])
    if motd then
    ...
    curWinW, curWinH = guiGetSize(GUIEditor.memo[1], false)

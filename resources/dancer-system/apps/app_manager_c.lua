...
function openAppsWindow(apps1, quests1)
	...
	if gui1.main and isElement(gui1.main) then
		guiSetText(gui1.main, "AMG Gaming Application Manager v3.2")
		guiSetEnabled(gui1.main, true)
	else
		local w, h = 740,474
		local x, y = (screenWidth-w)/2, (screenHeight-h)/2
		gui1.main = guiCreateWindow(x,y,w,h,"AMG Gaming Application Manager v3.2 | LOADING..",false)
...
local function updateButton(  )
	if exports.integration:isPlayerSupporter(localPlayer) then
		...
		if count > 0 then
			if not appsbutton then
				local screenWidth, screenHeight = guiGetScreenSize()
				appsbutton = guiCreateButton( screenWidth-40, screenHeight-70, 30, 30, tostring( count ), false )
...
function updateApps()
	...
	addEventHandler( "onClientGUIDoubleClick", gui1.grid_pendingApps,
		function( button )
			if button == "left" and source == gui1.grid_pendingApps then
				local row, col = -1, -1
	...
	addEventHandler( "onClientGUIDoubleClick", gui1.grid_acceptedApps,
		function( button )
			if button == "left" and source == gui1.grid_acceptedApps then
				local row, col = -1, -1
	...
	addEventHandler( "onClientGUIDoubleClick", gui1.grid_deniedApps,
		function( button )
			if button == "left" and source == gui1.grid_deniedApps then
				local row, col = -1, -1

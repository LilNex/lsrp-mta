...
function startStep2(questsFromServer)
	...
	local margin = 30
	guiStep2.intro = guiCreateLabel(startX, startY, width, line*2, "قبل ان تصبح مؤهلا للعب في هذا السيرفر يجب عليك اجتياز هذا الاختبار القصير. (Section 2/2)", false )
...
function validateApp()
	...
	if (quest1>1) and (quest2>1) and (quest3>1) and (quest4>1) then
		guiSetText(guiStep2.intro, "يتم ارسال التقديم الخاص بك..\nProcessing application..الرجاء الانتضار!")
...
function resetNoti()
	if guiStep2.intro and isElement(guiStep2.intro) and guiGetVisible(guiStep2.intro) then
		guiSetText(guiStep2.intro, " قبل ان تصبح مؤهلا للعب في هذا السيرفر يجب عليك اجتياز هذا الاختبار القصير وتتعلم القوانين. (Section 2/2)")

...
addEvent("wewin", true)
addEventHandler("wewin", getRootElement(), playIt)

local function updateButton( )
	if getResourceFromName ( 'global' ) then
		if (exports.integration:isPlayerTrialAdmin( localPlayer ) and getElementData( localPlayer, "duty_admin" ) == 1) or getElementData(localPlayer, "duty_supporter") == 1 then
...
addEventHandler( "onClientElementDataChange", localPlayer,
	function( name ) 
		if name == "admin_level" or name == "duty_admin" or name == "account:gmlevel" then
			updateButton( )
		end
	end
)

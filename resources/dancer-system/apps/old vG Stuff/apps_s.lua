...
function tempFixForMigration(thePlayer)
	if exports.integration:isPlayerTrialAdmin(thePlayer) then
		local counter = 0
		for key, value in ipairs(getElementsByType("player")) do
			if getElementData(value, "loggedin") then
				exports.anticheat:changeProtectedElementDataEx(value, "account:seamless:validated", true, true)
				counter = counter + 1
			end
		end
		outputChatBox("The migration fix was applied to " ..  counter .. " people.", thePlayer, 0, 255, 0)
		outputDebugString("The migration fix was applied to " ..  counter .. " people.")
	end
end
addCommandHandler("fixmigration", tempFixForMigration, false, false)

function win (p, c, t)
	local tp = exports.global:findPlayerByPartialNick(p, t)
	if tp then
		triggerClientEvent(tp, "wewin", tp)
	end
end
addCommandHandler("win", win, false, false)
...
local function updateAppCount( )
	...
	if result then
		exports.anticheat:changeProtectedElementDataEx( getResourceRootElement( ), "openapps", tonumber( result.count ) )
...
addEventHandler( "apps:update", getRootElement( ),
	function( account, state, reason )
		if (exports.integration:isPlayerTrialAdmin( client ) or exports.integration:isPlayerSupporter( client )) and ( state == 2 or state == 3 ) and tonumber( account.id ) and tonumber(account.appid )then
...
addEventHandler( "apps:showhistory", getRootElement( ),
	function( account )
		if (exports.integration:isPlayerTrialAdmin( client ) or exports.integration:isPlayerSupporter( client )) and tonumber(account.id) then
...
addEventHandler( "apps:show", getRootElement( ), 
	function( id )
		if exports.integration:isPlayerTrialAdmin( client ) or  exports.integration:isPlayerSupporter( source ) then

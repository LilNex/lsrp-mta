		...
		if not (targetPlayer) or not (...) then
			...
			if targetPlayer then
				...
				elseif (logged==1) then
					...
					local desc = look[5]
					call( getResourceFromName( "realism-system" ), "addCharacterKillBody", x, y, z, rotation, skin, getElementData(targetPlayer, "dbid"), targetPlayerName, getElementInterior(targetPlayer), getElementDimension(targetPlayer), getElementData(targetPlayer, "age"), getElementData(targetPlayer, "race"), getElementData(targetPlayer, "weight"), getElementData(targetPlayer, "height"), desc, info, getElementData(targetPlayer, "gender"))
...
local factionType = getElementData(theTeam, "type")
	...
	elseif (factionType==4) and not exports.integration:isPlayerTrialAdmin(thePlayer) then -- LSFD Bury
		if not (...) then
			outputChatBox("SYNTAX: /" .. commandName .. " [Full Player Name]", thePlayer, 255, 194, 14)
		else
			local targetPlayer = table.concat({...}, "_")
			...
			elseif (mysql:num_rows(result)==0) then
				...
				elseif cked == 2 then
					...
					outputChatBox(targetPlayer .. " was buried.", thePlayer, 0, 255, 0)
					exports.logs:logMessage("[/BURY] LSES MEMBER " .. getElementData(thePlayer, "account:username") .. "/".. getPlayerName(thePlayer) .." buried ".. targetPlayer , 4)

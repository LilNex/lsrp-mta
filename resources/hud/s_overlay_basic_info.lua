function showStats(thePlayer, commandName, targetPlayerName)
	...
	--FINANCE
	local currentGC = exports.mysql:query_fetch_assoc("SELECT `credits` FROM `accounts` WHERE `id`='"..getElementData(thePlayer, "account:id").."'  LIMIT 1")["credits"]
...
function outputChange(theKey, oldValue, newValue)
    ...
    if (getElementType(source) == "player") then 
		if theKey == "Hunger" or theKey == "Thirst" or theKey == "dirtymoney" then 
			hk.hunger = getElementData(source,"Hunger")
			hk.thirst = getElementData(source,"Thirst")
			hk.dirtymoney = getElementData(source,"dirtymoney")
			hk.health = math.floor(getElementHealth(source))
			triggerClientEvent(source,"hud:update",source,hk)
		end 
    end
end

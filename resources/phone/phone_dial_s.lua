...
function acceptPhoneCall()
	...
	setElementData(source, "InCall", true)

	local timerbroadcast = setTimer(startBroadCast, 500, 0, source, caller)
	return true
end
...
function startBroadCast (player, called)
	if player then
		-- outputDebugString(type(player))
		if getElementType(player) == "player" then

		if not getElementData(player, "InCall") or getElementData(player, "InCall") == false then
		
		--setElementData(player, "InCall", false)
			if isTimer ( timerbroadcast ) then
				 killTimer ( timerbroadcast )
		 	end
		
			return false
		end

		setPlayerVoiceBroadcastTo(player, called)
		setPlayerVoiceBroadcastTo(called, player)
	end
end

end

function StopBroadCast(player)

	if isTimer ( timerbroadcast ) then killTimer ( timerbroadcast ) end

end
addEvent("phone:StopBroadCast", true)
addEventHandler("phone:StopBroadCast", root, StopBroadCast)

...
function sortThreadsBySecdiff(fromPhone)
	...
	table.sort(sortedThreads[fromPhone], function(a, b) 
		if a[1][3] ~= nil or b[1][3] ~= nil then 
			return true 
		end
		return a[1][3] > b[1][3]
	end)
		...
		if #sortedThreads[phone] > 0 then
			...
			for i = 1, #sortedThreads[phone] do
				...
				if thread then
					...
					guiCreateLabel(0.5, 0.5, 1, 0.5, "It's lonely here..", true, wSMS)
					isEmpty = true
					break
				end
			end
...
function drawSMSComposer(xoffset, yoffset)
	if isPhoneGUICreated() then
		...
		smsComposerGUIs.sendGps = guiCreateButton(0.8, 0, 0.1, 0.1, "", true, wSMSComposer)
		guiSetAlpha(smsComposerGUIs.sendGps, 0.5)
		guiSetProperty(smsComposerGUIs.sendGps,"AlwaysOnTop","True")

...
function fetchMaximeCarShop ()
	...
	while true do
		...
		table.insert(maximeCarShop, row )
		--outputDebugString(maximeCarShop[1].vehmodel)
	end
...
function carshop_buyVehicle(paymentMethod)
	...
	if (paymentMethod == "cash") then
		if not exports.global:hasMoney(client, costCar) or costCar == 0 then
			if isOverlayDisabled then
				outputChatBox("Vous n'avez pas assez d'argent pour se payer ce véhicle..", client, 0, 255, 0)
			else
				exports.hud:sendBottomNotification(client, "L'argent est toujours un problème..", "Vous n'avez pas assez d'argent pour se payer ce véhicle..")
			end
			return false
		else
	...
	elseif (paymentMethod == "bank") then
		...
		if money < 0 or costCar == 0 then
			if isOverlayDisabled then 
				outputChatBox("Vous n'avez pas assez d'argent dans votre compte bancaire pour se payer ce véhicle..", client, 0, 255, 0)
			else
				exports.hud:sendBottomNotification(client, "L'argent est toujours un problème..", "Vous n'avez pas assez d'argent dans votre compte bancaire pour se payer ce véhicle..")
			end
		else
		...
		if isOverlayDisabled then 
			outputChatBox("Non.", client, 0, 255, 0)
		else
			exports.hud:sendBottomNotification(client, "Non!", "Juste no...")
		end
		return false
	end
	...
	local license = (getElementData(client, "license.car") == 1) and "" or "Vous n'avez pas de permis. Il est mieux pour vous de ne pas conduire."
	
	if isOverlayDisabled then
		outputChatBox("Félicitations, vous venez d'acheter un véhicule!", client)
		outputChatBox("Assurez-vous de /park votre véhicule quelque part, dans un délai d'une heure", client)
		outputChatBox("Cas échéant, votre véhicule sera supprimé et pourra pas être remboursé.", client)
		outputChatBox(license, client)
		outputChatBox("Changez la description du véhicule avec la commande : /ed or /editdescription.", client)
	else
		local content = {}
		table.insert(content, {"Assurez-vous de /park votre véhicule quelque part, dans un délai d'une heure ou le véhicule sera supprimé !"})
		table.insert(content, {license})
		table.insert(content, {"Changez la description du véhicule avec la commande : /ed ou /editdescription."})
		
		exports.hud:sendBottomNotification(client, "Félicitations, vous venez d'acheter un véhicule!", content, 30)
	end
		...
		if shopname and shopnicename and vehicle_shop_id and vehYear and vehBrand and vehModel and vehPrice then
			...
			if orderedVehID and tonumber(orderedVehID) == tonumber(vehicle_shop_id) then
				if exports.global:hasItem(player, 2) then 
					...
					outputChatBox("["..languagename.."] SMS de "..shopnicename..": Salut! Comme vous l'avez commandé, nous avons maintenant "..itemName.." en stock pour $"..exports.global:formatMoney(vehPrice)..". Venez voir ça!" , player, 120, 255, 80)
				end
			end
		end
	end

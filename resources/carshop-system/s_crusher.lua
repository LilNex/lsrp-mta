...
function resetPrice(theVehicle, matching)
	if isElement(theVehicle) and getElementType(theVehicle) == "vehicle" then
		if getElementData(theVehicle, "crushing") then
			...
			if thePlayer then
				outputChatBox("Revenez quand vous auriez changé d'avis!", thePlayer, 255, 194, 14)
		...
		if theVehicle and getVehicleOccupant(theVehicle) == thePlayer then -- player is driver
			if getElementData(theVehicle, "owner") == getElementData(thePlayer, "dbid") then
				...
				if getElementData(theVehicle, "requires.vehpos") then
					outputChatBox("(( Garez votre véhicule et revenezez. ))", thePlayer, 255, 0, 0)
				elseif price == 0 then
					outputChatBox("Quelque chose ne va pas bien avec cette " .. exports.global:getVehicleName(theVehicle) .. ".", thePlayer, 255, 0, 0 )
					outputChatBox("(( Contactez un admin pour vendre ce véhicule spécial. ))", thePlayer, 255, 194, 14)
				else
					setElementData(theVehicle, "crushing", price, false)
					triggerClientEvent(thePlayer, 'crusher:show', theVehicle, price)
				end
			else
				outputChatBox("Ce véhicule est enregistré? Non? Désolé fréro, je peux pas y toucher.", thePlayer, 255, 0, 0)
			end
		end
	end
end
		...
		if getElementData(theVehicle, "owner") == getElementData(thePlayer, "dbid") then
			...
			if price and price > 0 then
				...
				if result and dbid > 0 then
					...
					outputChatBox("Vous avez vendu votre " .. exports.global:getVehicleName(theVehicle) .. " au gouvernement pour $" .. exports.global:formatMoney(price) .. ".", thePlayer, 0, 255, 0)	
